...
if ($action=="updateconfig") {
	...
	if (isset($PGV_BLOCKS[$block[0]]["canconfig"]) && $PGV_BLOCKS[$block[0]]["canconfig"] && isset($PGV_BLOCKS[$block[0]]["config"]) && is_array($PGV_BLOCKS[$block[0]]["config"])) {
		...
		foreach($PGV_BLOCKS[$block[0]]["config"] as $config_name=>$config_value) {
			$config[$config_name] = ( isset( $_POST[$config_name] ) ) ? $_POST[$config_name] : "";
		}
	...
	?><script>parentrefresh();</script><?php
}


if ($action=="update") {
	if (0==count($main)) {  // for page to load need at least one block in main
		?><script>alert("At least one block needed on left side.  To have one column, move all blocks to the left.");window.close();</script><?php
		exit;
	}

	$existingids=array();

	$idrows=$gBitDb->query("SELECT b_id FROM {$TBLPREFIX}blocks WHERE b_username=? and b_location=?", array($name,'main') );
	...
	$idrows=$gBitDb->query("SELECT b_id FROM {$TBLPREFIX}blocks WHERE b_username=? and b_location=?", array($name,'right') );
	...
	foreach(array('main','right') as $column) {
		...
		foreach($existingids[$othercolumn] as $b_id) {
			if (!in_array($b_id, $othercolumnarr) && in_array($b_id, $columnarr)) {
				...
				$gBitDb->query("UPDATE {$TBLPREFIX}blocks SET b_location=? WHERE b_id=?", array($column, $b_id) );
			}
		}
		...
		foreach($existingids[$column] as $b_id) {
			if (!in_array($b_id, $columnarr) && in_array($b_id, $othercolumnarr)) {
				//$position = array_keys($othercolumnarr,$b_id);
				$gBitDb->query("UPDATE {$TBLPREFIX}blocks SET b_location=? WHERE b_id=?", array($othercolumn, $b_id));
			}
			if (!in_array($b_id, $columnarr) && !in_array($b_id, $othercolumnarr)) {
				$gBitDb->query("DELETE FROM {$TBLPREFIX}blocks WHERE b_id=?", array($b_id));
			}
		}
		...
		foreach($columnarr as $indexval => $b) {
			if (is_numeric($b)) { //existing block
				$gBitDb->query("UPDATE {$TBLPREFIX}blocks SET b_order=? WHERE b_id=?", array($indexval, $b));
			} else { //new block
				$gBitDb->query("INSERT INTO {$TBLPREFIX}blocks (b_id, b_username, b_location, b_order, b_name, b_config) VALUES (?, ?, ?, ?, ?, ?)", array(get_next_id("blocks", "b_id"), $name, $column, $indexval, $b, serialize($ublocks[$column][$b][1])));
			}
		}
	}
	...
	if ($setdefault) {  //copy this user's setting to defaultuser
		...
		$gBitDb->query("DELETE FROM {$TBLPREFIX}blocks WHERE b_username=?", array("defaultuser"));

		unset($existingids);
		$idrows=$gBitDb->query("SELECT b_id FROM {$TBLPREFIX}blocks WHERE b_username=?", array($name) );
		...
		foreach($existingids as $b_id) {
			$idblock=$gBitDb->query("SELECT * FROM {$TBLPREFIX}blocks WHERE b_id=?", array($b_id) );
			$gBitDb->query("INSERT INTO {$TBLPREFIX}blocks (b_id, b_username, b_location, b_order, b_name, b_config) VALUES (?, ?, ?, ?, ?, ?)", array(get_next_id("blocks", "b_id"), "defaultuser", $idblock[0]->b_location, $idblock[0]->b_order, $idblock[0]->b_name, $idblock[0]->b_config));
		}
	}
