...
function myplot($mytitle, $n, $xdata, $xtitle, $ydata, $ytitle, $legend) {
	...
	$stop = ( $male_female == 1 ) ? 2 : count( $ydata );
	$yprocentmax = 0;
	if ($percentage) {
		for ( $i = 0; $i < $stop; $i++ ) {
			$ytotal = 0;
		...
		$ymax = $yprocentmax;
		$scalefactor = ( $ymax > 0 ) ? 100.0 / $ymax : 0;
		$datastring = "chd=t:";
		for ( $i = 0; $i < $stop; $i++ ) {
			if (isset( $ydata[$i] )) {
				for ( $j = 0; $j < count( $ydata[$i] ); $j++ ) {
	...
	$imgurl = PHPGEDVIEW_PKG_URL."chart.php?cht=bvg&chs=950x300&chf=bg,s,ffffff00|c,s,ffffff00&chtt=" . $title . "&" . $datastring . "&" . $colorstring . "&chbh=";
	if (count( $ydata ) > 3) {
		$imgurl .= "5,1";
	}
...
function set_params( $current, $indfam, $xg, $zg, $titstr, $xt, $yt, $gx, $gz, $myfunc )
{
	...
	if ($x_as == $current) {
		...
		elseif ($y_as == 202) {
			...
			$male_female = false;
			$xtitle .= $pgv_lang["stplmf"];
			$zgiven = false;
			$xtitle .= $pgv_lang["stplipot"];
			$zmax = 1;
			$zgrenzen[0] = 100000;
		}
		elseif ($z_as == 301) {
...
if ($action == "update") {
	$x_as = $_POST["x-as"];
	$y_as = $_POST["y-as"];
	$z_as = isset( $_POST["z-as"] ) ? $_POST["z-as"] : 300;
	...
	$zgp = isset( $_POST["zas-grenzen-periode"] ) ? $_POST["zas-grenzen-periode"] : 0;
