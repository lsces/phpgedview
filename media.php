...
if (isset( $_REQUEST['xref'] ))
	$xref = $_REQUEST['xref'];

// If the user has overridden the defaults or changed the previous specifications for
// Sort sequence, Thumbnail option, or Filter text save the new values for future use
if (isset( $_POST['sortby'] )) {
	$sortby = safe_POST( 'sortby', 'file', 'title' );
	$_SESSION['MMsortby'] = $sortby;
}
else
	$sortby = $_SESSION['MMsortby'];
if (isset( $_POST['showthumb'] )) {
	$showthumb = safe_POST( 'showthumb', '1', '0' );
	$_SESSION['MMshowthumb'] = $showthumb;
}
else
	$showthumb = $_SESSION['MMshowthumb'];
if (isset( $_POST['filter'] )) {
	$filter = safe_POST( 'filter', PGV_REGEX_NOSCRIPT );
	$_SESSION['MMfilter'] = $filter;
}
else
	$filter = $_SESSION['MMfilter'];
...
function showchanges() {
	window.location = '<?php echo PGV_SCRIPT_NAME . "?show_changes=yes&directory=" . $directory . "&level=" . $level . "&subclick=" . $subclick; ?>';
}
...
if (check_media_structure()) {
	...
	if (!dir_is_writable( $MEDIA_DIRECTORY ) || !$MULTI_MEDIA) {
		...
		show_mediaUpload_form( 'media.php' );		// We have the green light to upload media, print the form
	}
	print "</div><br />";
	...
	if ($action == "deletedir") {
		...
		if ($clean) {
			...
			$resdir_fw = true;
			$resthumb_fw = true;
			if (file_exists( filename_decode( $directory . "index.php" ) ))
				@unlink( filename_decode( $directory . "index.php" ) );
			if (@is_dir( filename_decode( $directory ) ))
				$resdir = @rmdir( filename_decode( substr( $directory, 0, -1 ) ) );
			if (file_exists( filename_decode( $thumbdir . "index.php" ) ))
				@unlink( filename_decode( $thumbdir . "index.php" ) );
			if (@is_dir( filename_decode( $thumbdir ) ))
				$resthumb = @rmdir( filename_decode( substr( $thumbdir, 0, -1 ) ) );
			if (file_exists( filename_decode( $directory_fw . "index.php" ) ))
				@unlink( filename_decode( $directory_fw . "index.php" ) );
			if (@is_dir( filename_decode( $directory_fw ) ))
				$resdir_fw = @rmdir( filename_decode( substr( $directory_fw, 0, -1 ) ) );
			if (file_exists( filename_decode( $thumbdir_fw . "index.php" ) ))
				@unlink( filename_decode( $thumbdir_fw . "index.php" ) );
			if (@is_dir( filename_decode( $thumbdir_fw ) ))
				$resthumb_fw = @rmdir( filename_decode( substr( $thumbdir_fw, 0, -1 ) ) );
	...
	if ($action == "deletefile") {
		...
		while ( $row = $rows->fetchRow() ) {
			...
			while ( $i >= 0 && $j >= 0 ) {
				...
				$i--;
				$k++;
				if ($k > $MEDIA_DIRECTORY_LEVELS)
					break;
			}
			if ($match) {
				if ($row["m_gedfile"] != PGV_GED_ID)
					$onegedcom = false;
				else
					$xrefs[] = $row["m_media"];
			}
		...
		foreach ( $xrefs as $ind => $xref ) {
			...
			if ($xref != "") {
				...
				foreach ( $links as $pid => $type ) {
					$gedrec = isset( $pgv_changes[$pid . "_" . PGV_GEDCOM] ) ? find_updated_record( $pid, PGV_GED_ID ) : find_gedcom_record( $pid, PGV_GED_ID );
...
<form name="managemedia" id="managemedia" method="post" onsubmit="return checknames(this);" action="media.php">
	...
	<tr><td class="topbottombar" colspan="4"><?php print_help_link( "manage_media_help", "qm", "manage_media" );
	print $pgv_lang["manage_media"]; ?></td></tr>
	<?php

	$legendAlign = ( $TEXT_DIRECTION == 'ltr' ) ? 'align="right"' : 'align="left"';
	?>
	...
	<td class="optionbox wrap"><select name="sortby">
		<option value="title" <?php if ($sortby == 'title')
			print "selected=\"selected\""; ?>><?php print $factarray["TITL"]; ?></option>
		<option value="file" <?php if ($sortby == 'file')
			print "selected=\"selected\""; ?>><?php print $factarray["FILE"]; ?></option>
	</select></td>

	<!-- // NOTE: Row 1 right, Upload media files -->
	<td class="descriptionbox wrap width25" <?php print $legendAlign; ?>><?php print_help_link( "upload_media_help", "qm", "upload_media" );
	  print $pgv_lang["upload_media"]; ?></td>
	...
	<td class="optionbox wrap">
		...
		if ($showthumb)
			echo "1\" checked=\"checked\" onclick=\"document.managemedia.showthumb.value='0';";
		else
			echo "0\" onclick=\"document.managemedia.showthumb.value='1';"; ?>"/>
	</td>
	...
	<tr><td class="descriptionbox wrap width25" <?php print $legendAlign; ?>><?php print_help_link( "simple_filter_help", "qm", "filter" );
	  print $pgv_lang["filter"]; ?></td>
	<td class="optionbox wrap">
		<?php

		// Directory pick list
		if (empty( $directory )) {
			$directory = ( !empty( $_SESSION['upload_folder'] ) ) ? $_SESSION['upload_folder'] : $MEDIA_DIRECTORY;
		}
		if ($MEDIA_DIRECTORY_LEVELS > 0) {
			...
			print "<input name=\"directory\" type=\"hidden\" value=\"ALL\" />";
		// Text field for filter
		?>
		<input type="text" name="filter" value="<?php if ($filter)
			print $filter; ?>" /><br /><input type="submit" name="search" value="<?php print $pgv_lang["filter"]; ?>" onclick="this.form.subclick.value=this.name" />&nbsp;&nbsp;&nbsp;<input type="submit" name="all" value="<?php print $pgv_lang["display_all"]; ?>" onclick="this.form.subclick.value=this.name" /></td>
	...
	$tempURL = "media.php?";
	if (!empty( $subclick ))
		$tempURL .= "subclick={$subclick}&";
	$tempURL .= "action=thumbnail&all=yes&level={$level}&directory={$directory}";
	?>
...
<?php
		...
		if (!empty( $savedOutput ))
			print $savedOutput;		// Print everything we have saved up
		...
		if ($action == "filter") {
			...
			for ( $i = 0; $i < count( $levels ) - 2; $i++ )
				$pdir .= $levels[$i] . '/';
			if ($pdir != '') {
				$uplink = "<a href=\"" . encode_url( "media.php?directory={$pdir}&level=" . ( $level - 1 ) ) . "\">";
				if ($TEXT_DIRECTION == "rtl")
					$uplink .= getLRM();
				$uplink .= $pdir;
				if ($TEXT_DIRECTION == "rtl")
				...
				$uplink2 = "<a href=\"" . encode_url( "media.php?directory={$pdir}&level=" . ( $level - 1 ) ) . "\"><img class=\"icon\" src=\"" . $PGV_IMAGE_DIR . "/";
			...
			if (count( $dirs ) || $pdir != '') {
				...
				foreach ( $dirs as $indexval => $dir ) {
					...
					print "</td>";
					print "<td class=\"descriptionbox $TEXT_DIRECTION\">";
						print "<a href=\"".encode_url("media.php?directory={$directory}{$dir}/&level=".($level+1))."\">";
					...
					while (true) {
						...
						if ($media["CHANGE"]!="delete") {
							...
							if (!$isExternal && $objectCount<2) {
								$tempURL = "media.php?";
								$tempURL .= "action=deletefile&subclick={$subclick}&filename=".encrypt($media['FILE'])."&directory={$directory}&level={$level}&xref={$media['XREF']}&gedfile={$media['GEDFILE']}";
							...
							if (!empty($media["XREF"])) {
								$tempURL = "media.php?";
								$tempURL .= "action=removeobject&subclick={$subclick}&filename=".encrypt($media['FILE'])."&directory={$directory}&level={$level}&xref={$media['XREF']}&gedfile={$media['GEDFILE']}";
							...
							if ($media["LINKED"]) {
								$tempURL = "media.php?";
								$tempURL .= "action=removelinks&subclick={$subclick}&filename=".encrypt($media['FILE'])."&directory={$directory}&level={$level}&xref={$media['XREF']}&gedfile={$media['GEDFILE']}";
							...
							if ($USE_MEDIA_FIREWALL && ($media["EXISTS"] > 1)) {
								...
								$tempURL .= "&filename=".encrypt($media['FILE'])."&directory={$directory}&level={$level}&xref={$media['XREF']}&gedfile=".$media["GEDFILE"];
							...
							if (!$isExternal && (empty($media["THUMB"]) || !$media["THUMBEXISTS"])) {
								...
								if ($ext=="jpg" || $ext=="jpeg" || $ext=="gif" || $ext=="png") {
									$tempURL = "media.php?";
									$tempURL .= "action=thumbnail&all=no&level={$level}&directory={$directory}&filename=".encrypt($media["FILE"]);
						...
						if (!$isExternal && !$media["EXISTS"]) print "<span dir=\"ltr\">".PrintReady($media["FILE"])."</span><br /><span class=\"error\">".$pgv_lang["file_not_exists"]."</span><br />";
							...
							$tempText = ( substr( $mediaInfo['type'], 0, 4 ) == 'url_' ) 
								? 'URL' : PrintReady( $media["FILE"] );
							if (!empty($media["XREF"])) {
