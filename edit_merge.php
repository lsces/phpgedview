...
if ($action!="choose") {
	if ($gid1==$gid2 && $GEDCOM==$ged2) {
		...
		$gedrec1 = ( !isset( $pgv_changes[$gid1 . "_" . PGV_GEDCOM] ) ) ? find_gedcom_record( $gid1, PGV_GED_ID ) : find_updated_record( $gid1, get_id_from_gedcom( $GEDCOM ) );
		$gedrec2 = ( !isset( $pgv_changes[$gid2 . "_" . $ged2] ) ) ? find_gedcom_record( $gid2, get_id_from_gedcom( $ged2 ) ) : find_updated_record( $gid2, get_id_from_gedcom( $ged2 ) );
...
if ($action=="choose") {
	...
	echo '<script>document.getElementById("gid1").focus();</script>';
