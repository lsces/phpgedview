...
//Do we still need?
				...
				// print text
				ImageTtfText($image, (double)$fanChart['size'], (int)$tangle, (int)$tx, (int)$ty, $color, $fanChart['font'], $text);
	...
	if (!empty($SERVER_URL)) {
		ImageStringUp($image, 1, $fanw-10, (int)($fanh/3), $SERVER_URL, $color);
	} else {
		// PGV banner ;-)
		ImageStringUp($image, 1, $fanw-10, (int)($fanh/3), PGV_PHPGEDVIEW_URL, $color);
	}
