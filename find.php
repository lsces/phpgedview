...
$thumbget = $showthumb ? '&showthumb=true' : '';
...
if (empty($language_filter)) {
	$language_filter = !empty($_SESSION["language_filter"]) ? $_SESSION["language_filter"] : $lang_short_cut[$LANGUAGE];
}
...
if ($action=="filter") {
	...
	if ($type == "indi") {
		...
		if ($myindilist) {
			...
			usort($myindilist, array('\Bitweaver\Phpgedview\GedcomRecord', 'Compare'));
	...
	if ($type == "fam") {
		...
		if ($myfamlist) {
			$curged = $GEDCOM;
			echo "<td class=\"list_value_wrap $TEXT_DIRECTION\"><ul>";
			usort($myfamlist, array('\Bitweaver\Phpgedview\GedcomRecord', 'Compare'));
	...
	if ($type == "place") {
		...
		if ($all || $filter) {
			...
			if ($ctplace>0) {
				...
				uasort($revplacelist, "\Bitweaver\Phpgedview\stringsort");
				echo "<td class=\"list_value_wrap $TEXT_DIRECTION\"><ul>";
	...
	if ($type=="note") {
		echo '<table class="tabs_table ', $TEXT_DIRECTION, ' width90">';
		$mynotelist = $filter
			? search_notes($filter_array, array(PGV_GED_ID), 'AND', true)
			: get_note_list(PGV_GED_ID);
		if ($mynotelist) {
			usort($mynotelist, array('\Bitweaver\Phpgedview\GedcomRecord', 'Compare'));
	...
	if ($type=="source") {
		echo '<table class="tabs_table ', $TEXT_DIRECTION, ' width90">';
		$mysourcelist = $filter
			? search_sources($filter_array, array(PGV_GED_ID), 'AND', true)
			: get_source_list(PGV_GED_ID);
		if ($mysourcelist) {
			usort($mysourcelist, array('\Bitweaver\Phpgedview\GedcomRecord', 'Compare'));
			echo '<tr><td class="list_value_wrap"><ul>';
