...
if ($search == "yes") {
	...
	usort($medialist, "\Bitweaver\Phpgedview\mediasort"); // Reset numbering of medialist array
}
...
<form action="medialist.php" method="get">
	...
	<td class="optionbox wrap"><select name="sortby">
		<option value="title" <?php if ($sortby == 'title')
			echo "selected=\"selected\""; ?>><?php echo $factarray["TITL"]; ?></option>
		<option value="file" <?php if ($sortby == 'file')
			echo "selected=\"selected\""; ?>><?php echo $factarray["FILE"]; ?></option>
	</select></td>
	...
	<tr><td class="descriptionbox wrap width25" <?php echo $legendAlign; ?>><?php print_help_link( "simple_filter_help", "qm", "filter" );
	  echo $pgv_lang["filter"]; ?></td>
	<td class="optionbox wrap width25">
		<?php
		...
		if ($MEDIA_DIRECTORY_LEVELS > 0) {
			if (empty( $folder )) {
				$folder = ( !empty( $_SESSION['upload_folder'] ) ) ? $_SESSION['upload_folder'] : "ALL";
			}
			...
			echo "<input name=\"folder\" type=\"hidden\" value=\"ALL\" />";
	...
	<td class="descriptionbox wrap width25" <?php echo $legendAlign; ?>><?php print_help_link( "medialist_recursive_help", "qm", "medialist_recursive" );
	  echo $pgv_lang["medialist_recursive"]; ?></td>
	<td class="optionbox wrap width25">
...
if ($sortby == 'file')
	usort( $sortedMediaList, 'filesort' );
...
if ($ct > 0) {
	...
	for ( $i = 0; $i < $count; $i++ ) {
		...
		if ($name == "") {
			//$showFile = false;
			$name = ( $isExternal ) ? "URL" : basename( $media["FILE"] );
		}
