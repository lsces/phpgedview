...
for($i= $controller->treesize - 1; $i>=0; $i--) {
	...
	if (($curgen > 0 && $talloffset > 1) || (($curgen > $talloffset) && ($curgen < $controller->PEDIGREE_GENERATIONS))) {
		if ($i%2==1) {
			if ($SHOW_EMPTY_BOXES || ($controller->treeid[$i]) || ($controller->treeid[$i+1])) {
				$vlength = ( $talloffset < 2 ) ? $prevyoffset - $yoffset : $prevxoffset - $xoffset;
	...
	if (!empty($controller->treeid[$i]) || $SHOW_EMPTY_BOXES) {
		...
		$zindex = ( $talloffset == 2 ) ? $PEDIGREE_GENERATIONS - $curgen : 0;
...
if ($controller->rootPerson->canDisplayDetails()) {
	...
	if (count($famids)>0) {
		...
		switch ($talloffset) {
			case 0:
				$addxoffset = ( $PEDIGREE_GENERATIONS < 6 ) ? 60 * ( 5 - $PEDIGREE_GENERATIONS ) : 0;
				echo $addxoffset, "px; top:", $yoffset, "px; width:10px; height:10px; \">";
				break;
			case 1:
				if ($PEDIGREE_GENERATIONS < 4)
					$basexoffset += 60;
		...
		if ($view!="preview") {
			if ($talloffset < 2) {
				switch ($TEXT_DIRECTION) {
					case "rtl":
						echo "<a href=\"javascript: ", $pgv_lang["show"], "\" onclick=\"togglechildrenbox(); return false;\" onmouseover=\"swap_image('larrow', 1);\" onmouseout=\"swap_image('larrow', 1);\">";
						break;
					default:
						echo "<a href=\"javascript: ", $pgv_lang["show"], "\" onclick=\"togglechildrenbox(); return false;\" onmouseover=\"swap_image('larrow', 0);\" onmouseout=\"swap_image('larrow', 0);\">";
						break;
				}
				switch ($TEXT_DIRECTION) {
					case "rtl":
						echo "<img id=\"larrow\" src=\"", $PGV_IMAGE_DIR, "/", $PGV_IMAGES["rarrow"]["other"], "\" border=\"0\" alt=\"\" />";
						break;
					default:
						echo "<img id=\"larrow\" src=\"", $PGV_IMAGE_DIR, "/", $PGV_IMAGES["larrow"]["other"], "\" border=\"0\" alt=\"\" />";
						break;
				}
			}
		...
		foreach($famids as $ind=>$family) {
			if ($family!=null) {
				...
				$spid = ( $controller->rootid != $husb ) ? $family->getHusband() : $family->getWife();
				if (!empty($spid)) {
