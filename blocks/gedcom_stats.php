...
$PGV_BLOCKS['print_gedcom_stats']['config']   = [
	'cache'                => 1,
...
function print_gedcom_stats($block=true, $config='', $side='', $index='') {
	...
	if ($PGV_BLOCKS['print_gedcom_stats']['canconfig']) {
		if ($ctype=='gedcom' && PGV_USER_GEDCOM_ADMIN || $ctype=='user' && PGV_USER_ID) {
			$name = $ctype == 'gedcom' ? PGV_GEDCOM : PGV_USER_NAME;
	...
	if ($ct>0) {
		...
		$software = ( $tt > 0 ) ? printReady( trim( $tmatch[1] ) ) : trim( $match[1] );
		if (!empty($software)) {
			$text = str_replace( [ '#SOFTWARE#', '#CREATED_SOFTWARE#' ], $software, $pgv_lang['gedcom_created_using']);
			$tt = preg_match('/2 VERS (.*)/', $softrec, $tmatch);
			$version = ( $tt > 0 ) ? printReady( trim( $tmatch[1] ) ) : '';
			$text = str_replace( [ '#VERSION#', '#CREATED_VERSION#' ], $version, $text);
			$content .= $text;
		}
	}
	if (preg_match('/1 DATE (.+)/', $head, $match)) {
		if (empty($software)) {
			$content.=str_replace( [ '#DATE#', '#CREATED_DATE#' ], $stats->gedcomDate(), $pgv_lang['gedcom_created_on']);
		} else {
			$content.=str_replace( [ '#DATE#', '#CREATED_DATE#' ], $stats->gedcomDate(), $pgv_lang['gedcom_created_on2']);
		}
	}
