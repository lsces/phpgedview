...
$PGV_BLOCKS['print_yahrzeit']['config']   = [
	'cache'         => 1,
	'days'          => 30,
	'infoStyle'     => 'style2',
	'allowDownload' => 'yes'
	...
	case "style2": // Table style
		...
		foreach ($yahrzeits as $yahrzeit) {
			if ($yahrzeit['jd']>=$startjd && $yahrzeit['jd']<$startjd+$config['days']) {
				...
				$content .= "<a name='{$yahrzeit['jd']}'>".$yahrzeit['date']->Display(true, NULL, [])."</a>";
				$content .= "</td>";
				...
				if ($config['allowDownload']=='yes') {
					...
					//TODO does this work??
					$content .= "<abbr class=\"dtstart\" title=\"".strip_tags($yahrzeit['date']->Display(false,'Ymd', []))."\"></abbr>";
				...
				$content .= "<td class=\"list_value_wrap\">";
				$content .= "<a href=\"".$url."\" class=\"list_item url\">".$td->Display(true, NULL, [ 'gregorian' ])."</a>"; // hCalendar:url
				$content .= "&nbsp;</td>";
...
function print_yahrzeit_config($config) {
	...
	foreach ( [ 'style1', 'style2' ] as $style) {
