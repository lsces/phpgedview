...
$PGV_BLOCKS["print_upcoming_events"]["config"]		= [
	"cache"         => 1,
	"days"          => 30,
	"filter"        => "all",
	"onlyBDM"       => "no",
...
function print_upcoming_events($block=true, $config="", $side='', $index='') {
	...
	$daysprint = $config["days"] ?? 30;
	$filter = $config["filter"] ?? "all";
	$onlyBDM = $config["onlyBDM"] ?? "no";
	$infoStyle = $config["infoStyle"] ?? "style2";
	$sortStyle = $config["sortStyle"] ?? "alpha";
	$allowDownload = $config["allowDownload"] ?? "yes";
