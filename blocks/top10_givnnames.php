...
$PGV_BLOCKS["print_block_givn_top10"]["config"]		= [
	"cache"=>7,
	"num"=>10,
	"infoStyle"=>"style2",
	"showUnknown"=>"yes"
...
function print_block_givn_top10($block=true, $config="", $side='', $index='') {
	...
	$infoStyle = $config["infoStyle"] ?? "style2";
	$showUnknown = $config["showUnknown"] ?? "yes";
	...
	case "style1":	// Output style 1:  Simple list style.  Better suited to left side of page.
		$padding = ( $TEXT_DIRECTION == 'ltr' ) ? 'padding-left: 15px' : 'padding-right: 15px';
		$params= [ 1, $config['num'], 'rcount' ];
		//List Female names
