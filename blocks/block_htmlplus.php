...
function print_htmlplus_block($block=true, $config='', $side='', $index='') {
	...
	if ($config['title'] == '' && $PGV_BLOCKS['print_htmlplus_block']['canconfig']) {
		if ($ctype=="gedcom" && PGV_USER_GEDCOM_ADMIN || $ctype=="user" && PGV_USER_ID) {
			$name = $ctype == "gedcom" ? str_replace( "'", "\'", $GEDCOM ) : PGV_USER_NAME;
			$content .= "<br />"
...
function print_htmlplus_block_config($config)
{
	...
	while(false !== ($entry = $d->read()))
	{
		if(strstr($entry, 'block_htmlplus_'))
		{
			...
			$templates[] = array(
				'filename'		=>$entry,
				'title'			=> $pgv_lang[$bits[0]] ?? $bits[0],
				'description'	=> $pgv_lang[$bits[1]] ?? $bits[1],
	...
	if(count($gedcoms) > 1)
	{
		$sel_current = $config['gedcom'] == '__current__' ? ' selected="selected"' : '';
		$sel_default = $config['gedcom'] == '__default__' ? ' selected="selected"' : '';
		print "\t<tr>\n\t\t<td class=\"descriptionbox wrap width33\">"
		...
		{
			$sel = $ged_name == $config['gedcom'] ? ' selected="selected"' : '';
	...
	// compatibility mode
	$compat = $config['compat'] == 1 ? ' checked="checked"' : '';
	print "\t<tr>\n\t\t<td class=\"descriptionbox wrap width33\">"
	...
	// extended features
	$ui = $config['ui'] == 1 ? ' checked="checked"' : '';
	print "\t<tr>\n\t\t<td class=\"descriptionbox wrap width33\">"
