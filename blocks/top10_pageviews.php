...
$PGV_BLOCKS["top10_pageviews"]["config"]	= [
	"cache"           => 1,
	"num"             => 10,
...
function top10_pageviews($block=true, $config="", $side='', $index='') {
	global $TBLPREFIX, $gBitDb, $pgv_lang, $PGV_BLOCKS, $ctype, $PGV_IMAGES, $PGV_IMAGE_DIR, $SHOW_COUNTER, $SHOW_SOURCES, $TEXT_DIRECTION;

	if (empty($config)) {
		$config = $PGV_BLOCKS["top10_pageviews"]["config"];
	}

	$CountSide = $config["count_placement"] ?? "left";
