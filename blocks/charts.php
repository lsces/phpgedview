...
function print_charts_block($block = true, $config="", $side='', $index='') {
	...
	if (empty($config["pid"])) {
		$config["pid"] = ( !PGV_USER_ID ) ? $PEDIGREE_ROOT_ID : ( ( PGV_USER_GEDCOM_ID ) ? PGV_USER_GEDCOM_ID : $PEDIGREE_ROOT_ID );
	}
	...
	$show_full = ( $config["details"] == "no" ) ? 0 : 1;
...
?>
	...
	<td class="optionbox">
		<select name="type">
			...
			<?php if (file_exists(PGV_ROOT.'includes/classes/TreeNav.php')) { ?>
