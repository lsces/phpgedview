...
if ($action=="sendFiles") {
	...
	if (isset($indi_rec)) {
		...
		while ($row = $statement->fetchRow()) {
			if ($no_private_links) {
				if (displayDetailsById($row['i_id'], "INDI", true)) {
					echo "	<url>\n";
					echo "		<loc>", PGV_SERVER_NAME, PGV_SCRIPT_PATH, "individual.php?pid=", $row['i_id'], "&amp;ged=", urlencode($gedcom_name), "</loc>\n";
					$arec = get_sub_record(1, "1 CHAN", $row['i_gedcom'], 1);
	...
	if (isset($fam_rec)) {
		...
		while ($row=$statement->fetchRow()) {
			if ($no_private_links) {
				if (displayDetailsById($row['f_id'], "FAM", true)) {
					echo "	<url>\n";
					echo "		<loc>", PGV_SERVER_NAME, PGV_SCRIPT_PATH, "family.php?famid=", $row['f_id'], "&amp;ged=", urlencode($gedcom_name), "</loc>\n";
					$arec = get_sub_record(1, "1 CHAN", $row['f_gedcom'], 1);
					if (!empty($arec) && preg_match("/2 DATE (.*)/", $arec, $datematch))
						echo "		<lastmod>", date("Y-m-d", strtotime($datematch[1])), "</lastmod>\n";
					echo "		<changefreq>", $indirec_update, "</changefreq>\n";
					echo "		<priority>0.", $indirec_priority, "</priority>\n";
