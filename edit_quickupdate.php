...
$update_CHAN = isset($_POST["preserve_last_changed"]) && $_POST["preserve_last_changed"] == "on" ? false : true;

$addfacts = preg_split("/[,; ]/", $QUICK_ADD_FACTS);
usort($addfacts, '\Bitweaver\Phpgedview\factsort');
$reqdfacts = preg_split("/[,; ]/", $QUICK_REQUIRED_FACTS);

$famaddfacts = preg_split("/[,; ]/", $QUICK_ADD_FAMFACTS);
usort($famaddfacts, '\Bitweaver\Phpgedview\factsort');
...
if (!PGV_USER_CAN_EDIT) {
	...
	foreach ($famids as $famid) {
		$famrec = ( !isset( $pgv_changes[$famid . "_" . PGV_GEDCOM] ) ) ? find_family_record( $famid, PGV_GED_ID ) : find_updated_record( $famid, PGV_GED_ID );
...
$gedrec = ( !isset( $pgv_changes[$pid . "_" . PGV_GEDCOM] ) ) ? find_gedcom_record( $pid, PGV_GED_ID ) : find_updated_record( $pid, PGV_GED_ID );
...
if ($action == "update") {
	function check_updated_facts( $i, &$famrec, $TAGS, $prefix )
	{
		...
		$repeat_tags = [];
		$var = "{$prefix}{$i}DESCS";
		$DESCS = ( !empty( $_POST[$var] ) ) ? $_POST[$var] : [];
		$var = "{$prefix}{$i}DATES";
		$DATES = ( !empty( $_POST[$var] ) ) ? $_POST[$var] : [];
		$var = "{$prefix}{$i}PLACS";
		$PLACS = ( !empty( $_POST[$var] ) ) ? $_POST[$var] : [];
		$var = "{$prefix}{$i}TEMPS";
		$TEMPS = ( !empty( $_POST[$var] ) ) ? $_POST[$var] : [];
		$var = "{$prefix}{$i}RESNS";
		$RESNS = ( !empty( $_POST[$var] ) ) ? $_POST[$var] : [];
		$var = "{$prefix}{$i}REMS";
		$REMS = ( !empty( $_POST[$var] ) ) ? $_POST[$var] : [];
		$var = "F{$i}MARRY";
		$FMARRY = ( !empty( $_REQUEST[$var] ) ) ? $_REQUEST[$var] : "";
		$var = "F{$i}DIVY";
		$FDIVY = ( !empty( $_REQUEST[$var] ) ) ? $_REQUEST[$var] : "";
		...
		for ( $j = 0; $j < count( $TAGS ); $j++ ) {
			if (!empty( $TAGS[$j] )) {
				...
				if (( empty( $DATES[$j] ) ) && ( empty( $PLACS[$j] ) ) && ( empty( $TEMPS[$j] ) ) && ( empty( $RESNS[$j] ) )) {
					$factrec = ( !in_array( $fact, $typefacts ) ) ? "1 $fact" : "1 EVEN\n2 TYPE $fact";
					if (!empty( $DESCS[$j] )) {
						$factrec .= " $DESCS[$j]\n";
					}
					elseif (!empty( $FMARRY )) {
				...
				$lookup = ( !in_array( $fact, $typefacts ) ) ? "1 $fact" : "1 EVEN\n2 TYPE $fact\n";
				...
				if ($pos1 !== false) {
					...
					if ($noupdfact) {
						...
						if ($REMS[$j] == 1) {
							...
							$factrec = $oldfac;
							if (!empty( $DESCS[$j] )) {
								$factrec = ( strstr( $factrec, "1 $fact" ) ) ? preg_replace( "/1 $fact.*/", "1 $fact $DESCS[$j]", $factrec ) : $factrec . "\n1 $fact $DESCS[$j]";
							}
							if (!empty($DATES[$j])) {
								$factrec = ( strstr( $factrec, "\n2 DATE" ) ) ? preg_replace( "/2 DATE.*/", "2 DATE $DATES[$j]", $factrec ) : $factrec . "\n2 DATE $DATES[$j]";
							}
							if (!empty( $PLACS[$j] )) {
								$factrec = ( strstr( $factrec, "\n2 PLAC" ) ) ? preg_replace( "/2 PLAC.*/", "2 PLAC $PLACS[$j]", $factrec ) : $factrec . "\n2 PLAC $PLACS[$j]";
							}
							if (!empty($TEMPS[$j])) {
								$factrec = ( strstr( $factrec, "\n2 TEMP" ) ) ? preg_replace( "/2 TEMP.*/", "2 TEMP $TEMPS[$j]", $factrec ) : $factrec . "\n2 TEMP $TEMPS[$j]";
							}
							if (!empty($RESNS[$j])) {
								$factrec = ( strstr( $factrec, "\n2 RESN" ) ) ? preg_replace( "/2 RESN.*/", "2 RESN $RESNS[$j]", $factrec ) : $factrec . "\n2 RESN $RESNS[$j]";
							}
	...
	if (isset( $_REQUEST['SURN'] ))
		$SURN = $_REQUEST['SURN'];
	if (isset( $_REQUEST['MRSURN'] ))
		$MRSURN = $_REQUEST['MRSURN'];
	if (isset( $GIVN ) || isset( $SURN ) || isset( $MRSURN )) {
		$namerec = trim( get_sub_record( 1, "1 NAME", $gedrec ) );
		if (!empty( $namerec )) {
			if (isset( $GIVN )) {
				//-- check if name line has a GIVN and a SURN
				$namerec = ( preg_match( "~1 NAME.+/.*/~", $namerec ) > 0 ) ? preg_replace( "/1 NAME.+\/(.*)\//", "1 NAME $GIVN /$1/", $namerec ) : preg_replace( "/1 NAME.+/", "1 NAME $GIVN", $namerec );
				if (strpos( $namerec, "2 GIVN" ) !== false)
					$namerec = preg_replace( "/2 GIVN.*/", "2 GIVN $GIVN\n", $namerec );
				else
					$namerec .= "\n2 GIVN $GIVN";
			}
			if (isset( $SURN )) {
				//-- check if name line has a GIVN and a SURN
				$namerec = ( preg_match( "~1 NAME.+/.*/~", $namerec ) > 0 ) ? preg_replace( "/1 NAME(.+)\/.*\//", "1 NAME$1/$SURN/", $namerec ) : preg_replace( "/1 NAME ([\w.\ -_]+)/", "1 NAME $1 /$SURN/\n", $namerec );
				if (preg_match( "/2 SPFX (.*)/", $namerec, $match ) > 0) {
					$SURN = str_replace( trim( $match[1] ) . " ", "", $SURN );
				}
				if (strpos( $namerec, "2 SURN" ) !== false)
					$namerec = preg_replace( "/2 SURN.*/", "2 SURN $SURN\n", $namerec );
				else
					$namerec .= "\n2 SURN $SURN";
			}
			//-- update the married surname
			if (isset( $MRSURN ) && !empty( $MRSURN )) {
				if (strpos( $namerec, "2 _MARNM" ) !== false)
					$namerec = preg_replace( "/2 _MARNM.*/", "2 _MARNM /$MRSURN/\n", $namerec );
				else
					$namerec .= "\n2 _MARNM /$MRSURN/";
			}
			...
			if ($pos1 !== false) {
				$pos2 = strpos( $gedrec, "\n1", $pos1 + 5 );
				$gedrec = ( $pos2 === false ) ? substr( $gedrec, 0, $pos1 ) . $namerec : substr( $gedrec, 0, $pos1 ) . $namerec . "\n" . substr( $gedrec, $pos2 + 1 );
			}
		}
		else
			$gedrec .= "\n1 NAME $GIVN /$SURN/\n2 GIVN $GIVN\n2 SURN $SURN\n2 _MARNM /$MRSURN/";
	...
	//-- rtl name update
	if (isset( $_REQUEST['HSURN'] ))
		$HSURN = $_REQUEST['HSURN'];
	if (isset( $_REQUEST['HGIVN'] ))
		$HGIVN = $_REQUEST['HGIVN'];
	...
	if (isset( $_REQUEST['RSURN'] ))
		$RSURN = $_REQUEST['RSURN'];
	if (isset( $_REQUEST['RGIVN'] ))
		$RGIVN = $_REQUEST['RGIVN'];
	...
	//-- check for new fact
	if (isset( $_REQUEST['newfact'] ))
		$newfact = $_REQUEST['newfact'];
	if (isset( $_REQUEST['newClipboardFact'] ))
		$newfact = $_REQUEST['newClipboardFact'];		// The name mis-match is deliberate
	if (isset( $_REQUEST['DATE'] ))
		$DATE = $_REQUEST['DATE'];
	if (isset( $_REQUEST['PLAC'] ))
		$PLAC = $_REQUEST['PLAC'];
	if (isset( $_REQUEST['TEMP'] ))
		$TEMP = $_REQUEST['TEMP'];
	if (isset( $_REQUEST['RESN'] ))
		$RESN = $_REQUEST['RESN'];
	if (isset( $_REQUEST['DESC'] ))
		$DESC = $_REQUEST['DESC'];
	if (!empty( $newfact )) {
		$factrec = ( !in_array( $newfact, $typefacts ) ) ? "1 $newfact" : "1 EVEN\n2 TYPE $newfact";
		if (!empty( $DESC ))
			$factrec .= " $DESC\n";
		else
			$factrec .= "\n";
	...
	if (isset( $_REQUEST['ADDR'] ))
		$ADDR = $_REQUEST['ADDR'];
	if (isset( $_REQUEST['ADR1'] ))
		$ADR1 = $_REQUEST['ADR1'];
	if (isset( $_REQUEST['POST'] ))
		$POST = $_REQUEST['POST'];
	if (isset( $_REQUEST['ADR2'] ))
		$ADR2 = $_REQUEST['ADR2'];
	if (isset( $_REQUEST['CITY'] ))
		$CITY = $_REQUEST['CITY'];
	if (isset( $_REQUEST['STAE'] ))
		$STAE = $_REQUEST['STAE'];
	...
	for ( $i = 1; $i <= count( $sfams ); $i++ ) {
		...
		if (isset( $_REQUEST['SGIVN' . $i] ))
			$sgivn = $_REQUEST['SGIVN' . $i];
		if (isset( $_REQUEST['SSURN' . $i] ))
			$ssurn = $_REQUEST['SSURN' . $i];
		if (isset( $_REQUEST['MSSURN' . $i] ))
			$mssurn = $_REQUEST['MSSURN' . $i];
		...
		if (!empty( $sgivn ) || !empty( $ssurn )) {
			...
			if (!empty( $mssurn ))
				$spouserec .= "2 _MARNM /" . $mssurn . "/\n";

			if (isset( $_REQUEST['HSGIVN' . $i] ))
				$hsgivn = $_REQUEST['HSGIVN' . $i];
			if (isset( $_REQUEST['HSSURN' . $i] ))
				$hssurn = $_REQUEST['HSSURN' . $i];
			if (!empty( $hsgivn ) || !empty( $hssurn )) {
				$spouserec .= "2 _HEB " . $hsgivn . " /" . $hssurn . "/\n";
			}
			if (isset( $_REQUEST['RSGIVN' . $i] ))
				$rsgivn = $_REQUEST['RSGIVN' . $i];
			if (isset( $_REQUEST['RSSURN' . $i] ))
				$rssurn = $_REQUEST['RSSURN' . $i];
			if (!empty( $rsgivn ) || !empty( $rssurn )) {
				$spouserec .= "2 ROMN " . $rsgivn . " /" . $rssurn . "/\n";
			}
			if (isset( $_REQUEST['SSEX' . $i] ))
				$ssex = $_REQUEST['SSEX' . $i];
			if (!empty( $ssex ))
				$spouserec .= "1 SEX " . $ssex . "\n";

			if (isset( $_REQUEST['BDATE' . $i] ))
				$bdate = $_REQUEST['BDATE' . $i];
			if (isset( $_REQUEST['BPLAC' . $i] ))
				$bplac = $_REQUEST['BPLAC' . $i];
			if (!empty( $bdate ) || !empty( $bplac )) {
				...
				if (!empty( $bplac ))
					$spouserec .= "2 PLAC " . $bplac . "\n";
				if (isset( $_REQUEST['BRESN' . $i] ))
					$bresn = $_REQUEST['BRESN' . $i];
				if (!empty( $bresn ))
					$spouserec .= "2 RESN " . $bresn . "\n";
			}
			if (isset( $_REQUEST['DDATE' . $i] ))
				$bdate = $_REQUEST['DDATE' . $i];
			if (isset( $_REQUEST['DPLAC' . $i] ))
				$bplac = $_REQUEST['DPLAC' . $i];
			if (!empty( $bdate ) || !empty( $bplac )) {
				$spouserec .= "1 DEAT\n";
				if (!empty( $bdate )) {
					$bdate = check_input_date( $bdate );
				...
				if (!empty( $bplac ))
					$spouserec .= "2 PLAC " . $bplac . "\n";
				if (isset( $_REQUEST['DRESN' . $i] ))
					$bresn = $_REQUEST['DRESN' . $i];
				if (!empty( $bresn ))
					$spouserec .= "2 RESN " . $bresn . "\n";
			}
			$spouserec .= "\n1 FAMS @$famid@\n";
		...
		//-- check for new fact
		$var = "F{$i}newfact";
		$newfact = ( !empty( $_REQUEST[$var] ) ) ? $_REQUEST[$var] : "";
		if (!empty( $newfact )) {
			$factrec = ( !in_array( $newfact, $typefacts ) ) ? "1 $newfact\n" : "1 EVEN\n2 TYPE $newfact\n";
			$var = "F" . $i . "DATE";
			$FDATE = ( !empty( $_REQUEST[$var] ) ) ? $_REQUEST[$var] : "";
			if (!empty( $FDATE )) {
				$FDATE = check_input_date( $FDATE );
				$factrec .= "2 DATE $FDATE\n";
			}
			$var = "F{$i}PLAC";
			$FPLAC = ( !empty( $_REQUEST[$var] ) ) ? $_REQUEST[$var] : "";
			if (!empty( $FPLAC ))
				$factrec .= "2 PLAC $FPLAC\n";
			$var = "F" . $i . "TEMP";
			$FTEMP = ( !empty( $_REQUEST[$var] ) ) ? $_REQUEST[$var] : "";
			if (!empty( $FTEMP ))
				$factrec .= "2 TEMP $FTEMP\n";
			$var = "F" . $i . "RESN";
			$FRESN = ( !empty( $_REQUEST[$var] ) ) ? $_REQUEST[$var] : "";
			if (!empty( $FRESN ))
				$factrec .= "2 RESN $FRESN\n";
			//-- make sure that there is at least a Y
			if (preg_match( "/\n2 \w*/", $factrec ) == 0)
				$factrec = "1 $newfact Y\n";
			$famrec .= "\n" . $factrec;
		...
		if (isset( $_REQUEST['CHIL'] ))
			$CHIL = $_REQUEST['CHIL'];
		if (!empty( $CHIL[$i] )) {
			$famupdate = true;
			$famrec .= "\n1 CHIL @" . $CHIL[$i] . "@";
			$childrec = ( !isset( $pgv_changes[$CHIL[$i] . "_" . PGV_GEDCOM] ) ) ? find_person_record( $CHIL[$i], PGV_GED_ID ) : find_updated_record( $CHIL[$i], PGV_GED_ID );
		...
		$var = "F" . $i . "CDEL";
		$fcdel = ( !empty( $_REQUEST[$var] ) ) ? $_REQUEST[$var] : "";
		if (!empty( $fcdel )) {
		...
		$cgivn = "";
		$var = "C" . $i . "GIVN";
		$cgivn = ( !empty( $_REQUEST[$var] ) ) ? $_REQUEST[$var] : "";
		$csurn = "";
		$var = "C" . $i . "SURN";
		$csurn = ( !empty( $_REQUEST[$var] ) ) ? $_REQUEST[$var] : "";
		if (!empty( $cgivn ) || !empty( $csurn )) {
			//-- first add the new child
			$childrec = "0 @REF@ INDI\n";
			$childrec .= "1 NAME $cgivn /$csurn/\n";
			if (!empty( $cgivn ))
				$childrec .= "2 GIVN $cgivn\n";
			if (!empty( $csurn ))
				$childrec .= "2 SURN $csurn\n";
			if (isset( $_REQUEST["HC[$i]GIVN"] ))
				$hsgivn = $_REQUEST["HC[$i]GIVN"];
			if (isset( $_REQUEST["HC[$i]SURN"] ))
				$hssurn = $_REQUEST["HC[$i]SURN"];
			if (!empty( $hsgivn ) || !empty( $hssurn )) {
				$childrec .= "2 _HEB " . $hsgivn . " /" . $hssurn . "/\n";
			}
			if (isset( $_REQUEST["RC[$i]GIVN"] ))
				$rsgivn = $_REQUEST["RC[$i]GIVN"];
			if (isset( $_REQUEST["RC[$i]SURN"] ))
				$rssurn = $_REQUEST["RC[$i]SURN"];
			if (!empty( $rsgivn ) || !empty( $rssurn )) {
			...
			if (!empty( $cdate ) || !empty( $cplac )) {
				...
				if (!empty( $cdate ))
					$childrec .= "2 DATE $cdate\n";
				if (!empty( $cplac ))
					$childrec .= "2 PLAC $cplac\n";
				$var = "C" . $i . "RESN";
				$cresn = "";
				if (!empty( $_REQUEST[$var] ))
					$cresn = $_REQUEST[$var];
				if (!empty( $cresn ))
					$childrec .= "2 RESN $cresn\n";
			}
			//--child death
			$var = "C" . $i . "DDATE";
			$cdate = "";
		...
		if ($famupdate && ( $famrec != $oldfamrec ))
			replace_gedrec( $famid, $famrec, $update_CHAN );
	}

	//--add new spouse name, birth, marriage
	if (isset( $_REQUEST['SGIVN'] ))
		$SGIVN = $_REQUEST['SGIVN'];
	if (isset( $_REQUEST['SSURN'] ))
		$SSURN = $_REQUEST['SSURN'];
	if (isset( $_REQUEST['MSSURN'] ))
		$MSSURN = $_REQUEST['MSSURN'];
	if (isset( $_REQUEST['SSEX'] ))
		$SSEX = $_REQUEST['SSEX'];
	if (!empty( $SGIVN ) || !empty( $SSURN )) {
		//-- first add the new spouse
		$spouserec = "0 @REF@ INDI\n";
		...
		if (isset( $_REQUEST['BPLAC'] ))
			$BPLAC = $_REQUEST['BPLAC'];
		if (isset( $_REQUEST['BRESN'] ))
			$BRESN = $_REQUEST['BRESN'];
		if (!empty( $BDATE ) || !empty( $BPLAC )) {
			$spouserec .= "1 BIRT\n";
			if (!empty( $BDATE ))
				$spouserec .= "2 DATE $BDATE\n";
		...
		if (isset( $_REQUEST['DPLAC'] ))
			$DPLAC = $_REQUEST['DPLAC'];
		if (isset( $_REQUEST['DRESN'] ))
			$DRESN = $_REQUEST['DRESN'];
		if (!empty( $DDATE ) || !empty( $DPLAC )) {
			$spouserec .= "1 DEAT\n";
			if (!empty( $DDATE ))
				$spouserec .= "2 DATE $DDATE\n";
			if (!empty( $DPLAC ))
				$spouserec .= "2 PLAC $DPLAC\n";
			if (!empty( $DRESN ))
				$spouserec .= "2 RESN $DRESN\n";
		}
		...
		if (empty( $spouserec ))
			$spouserec = find_person_record( $xref, PGV_GED_ID );
		$spouserec .= "\n1 FAMS @$newfamid@\n";
	...
	if (isset( $_REQUEST['MARRY'] ))
		$MARRY = $_REQUEST['MARRY'];
	if (isset( $_REQUEST['MDATE'] ))
		$MDATE = $_REQUEST['MDATE'];
	if (isset( $_REQUEST['MPLAC'] ))
		$MPLAC = $_REQUEST['MPLAC'];
	if (isset( $_REQUEST['MRESN'] ))
		$MRESN = $_REQUEST['MRESN'];
	if (!empty( $MDATE ) || !empty( $MPLAC ) || !empty( $MARRY )) {
		if (empty( $newfamid )) {
			$famrec = "0 @REF@ FAM\n";
			if (strpos( $gedrec, "1 SEX M" ) !== false)
				$famrec .= "1 HUSB @$pid@\n";
			else
				$famrec .= "1 WIFE @$pid@";
			$newfamid = append_gedrec( $famrec );
		...
		if (!empty( $MDATE ))
			$factrec .= "2 DATE $MDATE\n";
		if (!empty( $MPLAC ))
			$factrec .= "2 PLAC $MPLAC\n";
		if (!empty( $MRESN ))
			$factrec .= "2 RESN $MRESN\n";
		$famrec .= "\n" . $factrec;
	}

	//--add new child, name, birth
	if (isset( $_REQUEST['CGIVN'] ))
		$CGIVN = $_REQUEST['CGIVN'];
	if (isset( $_REQUEST['CSURN'] ))
		$CSURN = $_REQUEST['CSURN'];
	if (isset( $_REQUEST['CSEX'] ))
		$CSEX = $_REQUEST['CSEX'];
	if (isset( $_REQUEST['HCGIVN'] ))
		$HCGIVN = $_REQUEST['HCGIVN'];
	if (isset( $_REQUEST['HCSURN'] ))
		$HCSURN = $_REQUEST['HCSURN'];
	if (!empty( $CGIVN ) || !empty( $CSURN )) {
		//-- first add the new child
		$childrec = "0 @REF@ INDI\n";
		$childrec .= "1 NAME $CGIVN /$CSURN/\n";
		...
		if (isset( $_REQUEST['CPLAC'] ))
			$CPLAC = $_REQUEST['CPLAC'];
		if (isset( $_REQUEST['CRESN'] ))
			$CRESN = $_REQUEST['CRESN'];
		if (!empty( $CDATE ) || !empty( $CPLAC )) {
			$childrec .= "1 BIRT\n";
			$CDATE = check_input_date( $CDATE );
			if (!empty( $CDATE ))
		...
		if (isset( $_REQUEST['CDDATE'] ))
			$CDDATE = $_REQUEST['CDDATE'];
		if (isset( $_REQUEST['CDPLAC'] ))
			$CDPLAC = $_REQUEST['CDPLAC'];
		if (isset( $_REQUEST['CDRESN'] ))
			$CDRESN = $_REQUEST['CDRESN'];
		if (!empty( $CDDATE ) || !empty( $CDPLAC )) {
			$childrec .= "1 DEAT\n";
			$CDDATE = check_input_date( $CDDATE );
			if (!empty( $CDDATE ))
				$childrec .= "2 DATE $CDDATE\n";
			if (!empty( $CDPLAC ))
				$childrec .= "2 PLAC $CDPLAC\n";
			if (!empty( $CDRESN ))
				$childrec .= "2 RESN $CDRESN\n";
		}
		$cxref = append_gedrec( $childrec );
		...
		if (empty( $newfamid )) {
			...
			if (empty( $childrec ))
				$childrec = find_person_record( $cxref, PGV_GED_ID );
			$childrec .= "\n1 FAMC @$newfamid@\n";
	...
	for ( $j = 1; $j <= count( $cfams ); $j++ ) {
		...
		if (empty( $FATHER[$i] )) {
			...
			$smsurn = "";
			//--add new spouse name, birth
			if (isset( $_REQUEST["FGIVN$i"] ))
				$sgivn = $_REQUEST["FGIVN$i"];
			if (isset( $_REQUEST["FSURN$i"] ))
				$ssurn = $_REQUEST["FSURN$i"];
			if (isset( $_REQUEST["FMRSURN$i"] ))
				$smsurn = $_REQUEST["FMRSURN$i"];
			if (!empty( $sgivn ) || !empty( $ssurn )) {
				//-- first add the new spouse
				$spouserec = "0 @REF@ INDI\n";
				$spouserec .= "1 NAME " . $sgivn . " /" . $ssurn . "/\n";
				...
				if (!empty( $ssurn ))
					$spouserec .= "2 SURN " . $ssurn . "\n";
				if (!empty( $smsurn ))
					$spouserec .= "2 _MARNM /" . $smsurn . "/\n";
				$hsgivn = "";
				$hssurn = "";
				if (isset( $_REQUEST["HFGIVN$i"] ))
					$hsgivn = $_REQUEST["HFGIVN$i"];
				if (isset( $_REQUEST["HFSURN$i"] ))
					$hssurn = $_REQUEST["HFSURN$i"];
				if (!empty( $hsgivn ) || !empty( $hssurn )) {
					$spouserec .= "2 _HEB " . $hsgivn . " /" . $hssurn . "/\n";
				}
				$rsgivn = "";
				$rssurn = "";
				if (isset( $_REQUEST["RFGIVN$i"] ))
					$rsgivn = $_REQUEST["RFGIVN$i"];
				if (isset( $_REQUEST["RFSURN$i"] ))
					$rssurn = $_REQUEST["RFSURN$i"];
				if (!empty( $rsgivn ) || !empty( $rssurn )) {
					$spouserec .= "2 ROMN " . $rsgivn . " /" . $rssurn . "/\n";
				}
				$ssex = "";
				if (isset( $_REQUEST["FSEX$i"] ))
					$ssex = $_REQUEST["FSEX$i"];
				if (!empty( $ssex ))
					$spouserec .= "1 SEX " . $ssex . "\n";
				$bdate = "";
				$bplac = "";
				if (isset( $_REQUEST["FBDATE$i"] ))
					$bdate = $_REQUEST["FBDATE$i"];
				if (isset( $_REQUEST["FBPLAC$i"] ))
					$bplac = $_REQUEST["FBPLAC$i"];
				if (!empty( $bdate ) || !empty( $bplac )) {
					$spouserec .= "1 BIRT\n";
					$bdate = check_input_date( $bdate );
					if (!empty( $bdate ))
					...
					if (!empty( $bplac ))
						$spouserec .= "2 PLAC " . $bplac . "\n";
					$bresn = "";
					if (isset( $_REQUEST["FBRESN$i"] ))
						$bresn = $_REQUEST["FBRESN$i"];
					if (!empty( $bresn ))
						$spouserec .= "2 RESN " . $bresn . "\n";
				}
				$bdate = "";
				$bplac = "";
				if (isset( $_REQUEST["FDDATE$i"] ))
					$bdate = $_REQUEST["FDDATE$i"];
				if (isset( $_REQUEST["FDPLAC$i"] ))
					$bplac = $_REQUEST["FDPLAC$i"];
				if (!empty( $bdate ) || !empty( $bplac )) {
					$spouserec .= "1 DEAT\n";
					$bdate = check_input_date( $bdate );
					if (!empty( $bdate ))
					...
					if (!empty( $bplac ))
						$spouserec .= "2 PLAC " . $bplac . "\n";
					$bresn = "";
					if (isset( $_REQUEST["FDRESN$i"] ))
						$bresn = $_REQUEST["FDRESN$i"];
					if (!empty( $bresn ))
						$spouserec .= "2 RESN " . $bresn . "\n";
				}
				if (empty( $famid )) {
			...
			if (empty( $oldfamrec )) {
				...
				if (empty( $spouserec ))
					$spouserec = find_person_record( $FATHER[$i], PGV_GED_ID );
				$spouserec .= "\n1 FAMS @$famid@";
		...
		if (empty( $MOTHER[$i] )) {
			...
			$ssurn = "";
			$smsurn = "";
			if (isset( $_REQUEST["MGIVN$i"] ))
				$sgivn = $_REQUEST["MGIVN$i"];
			if (isset( $_REQUEST["MSURN$i"] ))
				$ssurn = $_REQUEST["MSURN$i"];
			if (isset( $_REQUEST["MMRSURN$i"] ))
				$smsurn = $_REQUEST["MMRSURN$i"];
			//--add new spouse name, birth
			if (!empty( $sgivn ) || !empty( $ssurn )) {
				//-- first add the new spouse
				$spouserec = "0 @REF@ INDI\n";
				...
				$hsgivn = "";
				$hssurn = "";
				if (isset( $_REQUEST["HMGIVN$i"] ))
					$hsgivn = $_REQUEST["HMGIVN$i"];
				if (isset( $_REQUEST["HMSURN$i"] ))
					$hssurn = $_REQUEST["HMSURN$i"];
				if (!empty( $hsgivn ) || !empty( $hssurn )) {
					$spouserec .= "2 _HEB " . $hsgivn . " /" . $hssurn . "/\n";
				}
				$rsgivn = "";
				$rssurn = "";
				if (isset( $_REQUEST["RMGIVN$i"] ))
					$rsgivn = $_REQUEST["RMGIVN$i"];
				if (isset( $_REQUEST["RMSURN$i"] ))
					$rssurn = $_REQUEST["RMSURN$i"];
				if (!empty( $rsgivn ) || !empty( $rssurn )) {
					$spouserec .= "2 ROMN " . $rsgivn . " /" . $rssurn . "/\n";
				}
				$ssex = "";
				if (isset( $_REQUEST["MSEX$i"] ))
					$ssex = $_REQUEST["MSEX$i"];
				if (!empty( $ssex ))
					$spouserec .= "1 SEX " . $ssex . "\n";
				$bdate = "";
				$bplac = "";
				if (isset( $_REQUEST["MBDATE$i"] ))
					$bdate = $_REQUEST["MBDATE$i"];
				if (isset( $_REQUEST["MBPLAC$i"] ))
					$bplac = $_REQUEST["MBPLAC$i"];
				if (!empty( $bdate ) || !empty( $bplac )) {
					$spouserec .= "1 BIRT\n";
					$bdate = check_input_date( $bdate );
					if (!empty( $bdate ))
					...
					if (!empty( $bplac ))
						$spouserec .= "2 PLAC " . $bplac . "\n";
					$bresn = "";
					if (isset( $_REQUEST["MBRESN$i"] ))
						$bplac = $_REQUEST["MBRESN$i"];
					if (!empty( $bresn ))
						$spouserec .= "2 RESN " . $bresn . "\n";
				}
				$bdate = "";
				$bplac = "";
				if (isset( $_REQUEST["MDDATE$i"] ))
					$bdate = $_REQUEST["MDDATE$i"];
				if (isset( $_REQUEST["MDPLAC$i"] ))
					$bplac = $_REQUEST["MDPLAC$i"];
				if (!empty( $bdate ) || !empty( $bplac )) {
					$spouserec .= "1 DEAT\n";
					$bdate = check_input_date( $bdate );
					if (!empty( $bdate ))
					...
					if (!empty( $bplac ))
						$spouserec .= "2 PLAC " . $bplac . "\n";
					$bresn = "";
					if (isset( $_REQUEST["MDRESN$i"] ))
						$bplac = $_REQUEST["MDRESN$i"];
					if (!empty( $bresn ))
						$spouserec .= "2 RESN " . $bresn . "\n";
				}
				if (empty( $famid )) {
		...
		$cgivn = "";
		$csurn = "";
		if (isset( $_REQUEST["C" . $i . "GIVN"] ))
			$cgivn = $_REQUEST["C" . $i . "GIVN"];
		if (isset( $_REQUEST["C" . $i . "SURN"] ))
			$csurn = $_REQUEST["C" . $i . "SURN"];
		if (!empty( $cgivn ) || !empty( $csurn )) {
			if (empty( $famid )) {
				//echo "HERE 8";
				$famid = append_gedrec( $famrec );
			...
			$hcsurn = "";
			if (isset( $_REQUEST["HC" . $i . "GIVN"] ))
				$hcgivn = $_REQUEST["HC" . $i . "GIVN"];
			if (isset( $_REQUEST["HC" . $i . "SURN"] ))
				$hcsurn = $_REQUEST["HC" . $i . "SURN"];
			if (!empty( $hcgivn ) || !empty( $hcsurn )) {
				$childrec .= "2 _HEB " . $hcgivn . " /" . $hcsurn . "/\n";
			}
			$rsgivn = "";
			...
			$csex = $_REQUEST["C{$i}SEX"] ?? "";
			if (!empty( $csex ))
				$childrec .= "1 SEX $csex\n";
			//-- child birth
			$cdate = $_REQUEST["C{$i}DATE"] ?? "";
			$var = "C{$i}PLAC";
			$cplac = $_REQUEST[$var] ?? "";
			if (!empty( $cdate ) || !empty( $cplac )) {
				$childrec .= "1 BIRT\n";
...
if ($action == "choosepid") {
	...
	$SEX = get_gedcom_value( "SEX", 1, $gedrec, 0, false );
	$child_surname = "";
	$GIVN = "";
	$SURN = "";
	$MRSURN = "";
	...
	if (!empty( $subrec )) {
		...
		if ($ct > 0)
			$MRSURN = trim( str_replace( "/", "", $match[1] ) );
		//else {
		//	$ct = preg_match("/1 NAME (.*)/", $subrec, $match);
		//	if ($ct>0) {
		//		$st = preg_match("~/(.*)/~", $match[1], $smatch);
		...
		$RGIVN = "";
		$RSURN = "";
		$hname = get_gedcom_value( "_HEB", 2, $subrec, 0, false );
		if (!empty( $hname )) {
		...
		$rname = get_gedcom_value( "ROMN", 2, $subrec, 0, false );
		if (!empty( $rname )) {
...
<table class="tabs_table">
	<tr>
		...
		<?php if (strpos( $gedrec, "1 SEX M" ) !== false)
			echo $pgv_lang["add_new_wife"];
		else
			echo $pgv_lang["add_new_husb"]; ?></a></td>
		<?php
...
<?php if ($NAME_REVERSE) { ?>
<tr>
	<td class="descriptionbox"><?php print_help_link( "edit_surname_help", "qm" );
			echo $factarray["SURN"]; ?></td>
...
<?php } ?>
<tr>
	<td class="descriptionbox"><?php print_help_link( "edit_given_name_help", "qm" );
	echo $factarray["GIVN"]; ?></td>
...
<?php if ($NAME_REVERSE) { ?>
<tr>
	<td class="descriptionbox"><?php print_help_link( "edit__HEB_SURN_help", "qm" );
					echo $pgv_lang["hebrew_surn"]; ?></td>
...
<?php } ?>
<tr>
	<td class="descriptionbox"><?php print_help_link( "edit__HEB_GIVN_help", "qm" );
			echo $pgv_lang["hebrew_givn"]; ?></td>
...
<?php if ($NAME_REVERSE) { ?>
<tr>
	<td class="descriptionbox"><?php print_help_link( "edit_ROMN_SURN_help", "qm" );
					echo $pgv_lang["roman_surn"]; ?></td>
...
<?php } ?>
<tr>
	<td class="descriptionbox"><?php print_help_link( "edit_ROMN_GIVN_help", "qm" );
			echo $pgv_lang["roman_givn"]; ?></td>
...
<?php } ?>
<tr>
	<td class="descriptionbox"><?php print_help_link( "edit_sex_help", "qm" );
	echo $pgv_lang["sex"]; ?></td>
	<td class="optionbox" colspan="3">
		<select name="GENDER" tabindex="<?php echo $tabkey; ?>">
			<option value="M"<?php if ($SEX == "M")
				echo " selected=\"selected\""; ?>><?php echo $pgv_lang["male"]; ?></option>
			<option value="F"<?php if ($SEX == "F")
				echo " selected=\"selected\""; ?>><?php echo $pgv_lang["female"]; ?></option>
			<option value="U"<?php if ($SEX == "U")
				echo " selected=\"selected\""; ?>><?php echo $pgv_lang["unknown"]; ?></option>
		</select>
	<?php $tabkey++; ?>
	</td>
</tr>
<?php
		...
		?>
<tr><td>&nbsp;</td></tr>
<tr><td class="topbottombar" colspan="4"><?php print_help_link( "quick_update_fact_help", "qm" );
		echo $pgv_lang["update_fact"]; ?></td></tr>
<tr>
	<td class="descriptionbox">&nbsp;</td>
	<td class="descriptionbox"><?php print_help_link( "def_gedcom_date_help", "qm" );
	echo $factarray["DATE"]; ?></td>
	<td class="descriptionbox"><?php print_help_link( "edit_PLAC_help", "qm" );
	echo $factarray["PLAC"]; ?></td>
...
<tr>
	...
	<?php
			...
			<td class="optionbox">
				...
				<input type="text" dir="ltr" tabindex="<?php echo $tabkey;
				$tabkey++; ?>" size="15" name="DATES[]" id="DATE<?php echo $f; ?>" onblur="valid_date(this);" value="<?php echo PrintReady( htmlspecialchars( $date, ENT_COMPAT, 'UTF-8' ) ); ?>" />&nbsp;<?php print_calendar_popup( "DATE$f" ); ?>
			</td>
		<?php
			...
			<td class="optionbox">
				<input type="text" size="35" tabindex="<?php echo $tabkey;
				$tabkey++; ?>" name="PLACS[]" id="place<?php echo $f; ?>" value="<?php echo PrintReady( htmlspecialchars( $plac, ENT_COMPAT, 'UTF-8' ) ); ?>" />
...
<tr><td class="optionbox">
	...
	function checkDesc(newfactSelect) {
		...
		var emptyfacts = "<?php foreach ( $emptyfacts as $ind => $efact )
			echo $efact, ","; ?>";
	...
	<?php
			...
			foreach ( $addfacts as $indexval => $fact ) {
				...
				if (!$found)
					echo "\t\t<option value=\"$fact\">", $factarray[$fact], "</option>\n";
			}
			?>
		</select>
...
<?php
		...
		if ($person && !$person->isDead() || !empty( $ADDR ) || !empty( $PHON ) || !empty( $FAX ) || !empty( $EMAIL )) { //-- don't show address for dead people
			?>
<tr><td>&nbsp;</td></tr>
<tr><td class="topbottombar" colspan="4"><?php print_help_link( "quick_update_address_help", "qm" );
				echo $pgv_lang["update_address"]; ?></td></tr>
<tr>
	<td class="descriptionbox">
	...
	<td class="optionbox" colspan="3">
		<?php if (!empty( $CITY ) && !empty( $POST )) { ?>
			...
			<table>
			<?php if (!empty( $_NAME )) { ?><tr><td><?php echo $factarray["NAME"]; ?></td><td><input type="text" <?php if ($TEXT_DIRECTION == "rtl" && !hasRTLText( $_NAME ))
					  echo "dir=\"ltr\""; ?> name="_NAME" size="35" value="<?php echo PrintReady( htmlspecialchars( strip_tags( $_NAME ), ENT_COMPAT, 'UTF-8' ) ); ?>" /></td></tr><?php } ?>
			<?php if (!empty( $ADDR )) { ?><tr><td><?php echo $factarray["ADDR"]; ?></td><td><input type="text" <?php if ($TEXT_DIRECTION == "rtl" && !hasRTLText( $ADR1 ))
					   echo "dir=\"ltr\""; ?> name="ADDR" size="35" value="<?php echo PrintReady( htmlspecialchars( strip_tags( preg_replace( '~(.*)\n(.*)~', '${1}', $ADDR ) ), ENT_COMPAT, 'UTF-8' ) ); ?>" /></td></tr><?php } ?>
			<tr><td><?php echo $factarray["ADR1"]; ?></td><td><input type="text" <?php if ($TEXT_DIRECTION == "rtl" && !hasRTLText( $ADR1 ))
				   echo "dir=\"ltr\""; ?> name="ADR1" size="35" value="<?php echo PrintReady( htmlspecialchars( strip_tags( $ADR1 ), ENT_COMPAT, 'UTF-8' ) ); ?>" /></td></tr>
			<tr><td><?php echo $factarray["ADR2"]; ?></td><td><input type="text" <?php if ($TEXT_DIRECTION == "rtl" && !hasRTLText( $ADR2 ))
				   echo "dir=\"ltr\""; ?> name="ADR2" size="35" value="<?php echo PrintReady( htmlspecialchars( strip_tags( $ADR2 ), ENT_COMPAT, 'UTF-8' ) ); ?>" /></td></tr>
			<tr><td><?php echo $factarray["CITY"]; ?></td><td><input type="text" <?php if ($TEXT_DIRECTION == "rtl" && !hasRTLText( $CITY ))
				   echo "dir=\"ltr\""; ?> name="CITY" value="<?php echo PrintReady( htmlspecialchars( strip_tags( $CITY ), ENT_COMPAT, 'UTF-8' ) ); ?>" />
			<?php echo $factarray["STAE"]; ?> <input type="text" <?php if ($TEXT_DIRECTION == "rtl" && !hasRTLText( $STAE ))
					echo "dir=\"ltr\""; ?> name="STAE" value="<?php echo PrintReady( htmlspecialchars( strip_tags( $STAE ), ENT_COMPAT, 'UTF-8' ) ); ?>" /></td></tr>
			<tr><td><?php echo $factarray["POST"]; ?></td><td><input type="text" <?php if ($TEXT_DIRECTION == "rtl" && !hasRTLText( $POST ))
				   echo "dir=\"ltr\""; ?> name="POST" value="<?php echo PrintReady( htmlspecialchars( strip_tags( $POST ), ENT_COMPAT, 'UTF-8' ) ); ?>" /></td></tr>
			<tr><td><?php echo $factarray["CTRY"]; ?></td><td><input type="text" <?php if ($TEXT_DIRECTION == "rtl" && !hasRTLText( $CTRY ))
				   echo "dir=\"ltr\""; ?> name="CTRY" value="<?php echo PrintReady( htmlspecialchars( strip_tags( $CTRY ), ENT_COMPAT, 'UTF-8' ) ); ?>" /></td></tr>
			</table>
...
<tr>
	...
	<td class="optionbox" colspan="3">
		<input type="text" dir="ltr" tabindex="<?php echo $tabkey;
		$tabkey++; ?>" name="PHON" size="20" value="<?php echo PrintReady( htmlspecialchars( $PHON, ENT_COMPAT, 'UTF-8' ) ); ?>" />
	</td>
</tr>
<tr>
		<td class="descriptionbox">
		...
		<td class="optionbox" colspan="3">
				<input type="text" dir="ltr" tabindex="<?php echo $tabkey;
				$tabkey++; ?>" name="FAX" size="20" value="<?php echo PrintReady( htmlspecialchars( $FAX, ENT_COMPAT, 'UTF-8' ) ); ?>" />
	</td>
</tr>
<tr>
	<td class="descriptionbox">
...
<?php
				...
				$subrecords = $family->getFacts( [ "HUSB", "WIFE", "CHIL" ] );
				$famfacts = [];
...
<?php if (empty( $spid )) { ?>
<tr><td>&nbsp;</td></tr>
<tr><td class="topbottombar" colspan="4"><?php print_help_link( "quick_update_spouse_help", "qm" );
						if (strpos( $gedrec, "1 SEX M" ) !== false)
							echo $pgv_lang["add_new_wife"];
						else
							echo $pgv_lang["add_new_husb"]; ?></td></tr>
<?php if ($NAME_REVERSE) { ?>
<tr>
	<td class="descriptionbox"><?php print_help_link( "edit_surname_help", "qm" );
							echo $factarray["SURN"]; ?></td>
...
<tr>
	...
	<td class="optionbox" colspan="3">
		<select name="SSEX<?php echo $i; ?>" tabindex="<?php echo $tabkey; ?>">
			<option value="M"<?php if (strpos( $gedrec, "1 SEX F" ) !== false)
				echo " selected=\"selected\""; ?>><?php echo $pgv_lang["male"]; ?></option>
			<option value="F"<?php if (strpos( $gedrec, "1 SEX M" ) !== false)
				echo " selected=\"selected\""; ?>><?php echo $pgv_lang["female"]; ?></option>
			<option value="U"<?php if (strpos( $gedrec, "1 SEX U" ) !== false)
				echo " selected=\"selected\""; ?>><?php echo $pgv_lang["unknown"]; ?></option>
		</select>
	<?php $tabkey++; ?>
	</td>
...
<?php
			...
			<tr>
				...
				<td class="optionbox"><input type="text" dir="ltr" tabindex="<?php echo $tabkey;
				$tabkey++; ?>" size="15" name="F<?php echo $i; ?>DATES[]" id="F<?php echo $i; ?>DATE<?php echo $f; ?>" onblur="valid_date(this);" value="<?php echo PrintReady( htmlspecialchars( $date, ENT_COMPAT, 'UTF-8' ) ); ?>" />&nbsp;<?php print_calendar_popup( "F[$i]DATE{$f}" ); ?></td>
				<?php if (empty( $temp ) && ( !in_array( $fact_tag, $nonplacfacts ) )) { ?>
					<td class="optionbox"><input type="text" size="35" tabindex="<?php echo $tabkey;
					$tabkey++; ?>" name="F<?php echo $i; ?>PLACS[]" id="F<?php echo $i; ?>place<?php echo $f; ?>" value="<?php echo PrintReady( htmlspecialchars( $plac, ENT_COMPAT, 'UTF-8' ) ); ?>" />
	...
	<td class="optionbox"><input type="text" dir="ltr" tabindex="<?php echo $tabkey; ?>" size="15" name="F<?php echo $i; ?>DATE" id="F<?php echo $i; ?>DATE" onblur="valid_date(this);" />&nbsp;<?php print_calendar_popup( "F", $i ); ?></td>
	<?php $tabkey++; ?>
...
<?php
	...
	<tr>
		...
		<td class="descriptionbox"><input type="hidden" name="F<?php echo $i; ?>CDEL" value="" /><?php echo $pgv_lang["remove"]; ?></td>
			...
			// NOTE: Add a child
			if (empty( $child_surname ))
				$child_surname = "";
			?>
<tr><td>&nbsp;</td></tr>
<tr><td class="topbottombar" colspan="4"><b><?php print_help_link( "quick_update_child_help", "qm" );
				echo $pgv_lang["add_new_chil"]; ?></b></td></tr>
<tr>
...
<tr>
	<td class="descriptionbox"><?php print_help_link( "edit_surname_help", "qm" );
					echo $factarray["SURN"]; ?></td>
	<td class="optionbox" colspan="3"><input size="50" type="text" tabindex="<?php echo $tabkey; ?>" name="C<?php echo $i; ?>SURN" value="<?php if (!empty( $child_surname ))
				  echo $child_surname; ?>" /></td>
</tr>
<?php $tabkey++; ?>
<?php } ?>
<tr>
...
<?php
		...
		print_quick_resn( "DRESN" );

		// NOTE: Marriage
		?>
<tr><td>&nbsp;</td></tr>
<tr>
	<td class="topbottombar" colspan="4"><?php print_help_link( "quick_update_marriage_help", "qm" );
	echo $factarray["MARR"]; ?></td>
</tr>
<tr><td class="descriptionbox">
	<?php print_help_link( "quick_update_marriage_help", "qm" );
	echo $factarray["MARR"]; ?>
	</td>
	...
	<?php $tabkey++; ?>
	<tr><td class="descriptionbox">
		<?php print_help_link( "def_gedcom_date_help", "qm" );
		echo $factarray["DATE"]; ?>
	</td>
...
<tr>
	<td class="descriptionbox"><?php print_help_link( "edit_surname_help", "qm" );
			echo $factarray["SURN"]; ?></td>
	<td class="optionbox" colspan="3"><input size="50" type="text" tabindex="<?php echo $tabkey; ?>" name="CSURN" value="<?php if (!empty( $child_surname ))
			 echo $child_surname; ?>" /></td>
</tr>
<?php $tabkey++; ?>
<?php } ?>
...
<?php } ?>
<tr>
	<td class="descriptionbox"><?php print_help_link( "edit_ROMN_GIVN_name_help", "qm" );
			echo $pgv_lang["roman_givn"]; ?></td>
...
<?php
				...
				$subrecords = $family ? $family->getFacts( [ "HUSB", "WIFE", "CHIL" ] ) : [];
				$famfacts = [];
				foreach ( $subrecords as $ind => $eventObj ) {
					$fact = $eventObj->getTag();
					$event = $eventObj->getDetail();
