...
uasort($countries, "\Bitweaver\Phpgedview\stringsort");
...
uasort($assorela, "\Bitweaver\Phpgedview\stringsort");
...
echo PGV_JS_START;
?>
	var locale_date_format='<?php echo preg_replace('/[^DMY]/', '', str_replace( [ 'J', 'F' ], [ 'D', 'M' ], strtoupper($DATE_FORMAT))); ?>';
...
if (!empty($pid)) {
	...
	if ($famid != "new") {
		$gedrec = (!isset($pgv_changes[$famid."_".PGV_GEDCOM])) ? find_gedcom_record($famid, PGV_GED_ID) : find_updated_record($famid, PGV_GED_ID);
...
elseif ($type=="FAM") {
	$record = (!empty($pid)) ? Family::getInstance($pid) : Family::getInstance($famid);
	echo "<b>", PrintReady($record->getFullName()), "</b><br />";
} elseif ($type=="SOUR") {
...
switch ($action) {
	case 'delete':
		...
		if (!empty($linenum)) {
			if ($linenum===0) {
				...
				$newged = (!is_numeric($linenum)) ? remove_subrecord($gedrec, $linenum, $mediaid) : remove_subline($gedrec, $linenum);
	...
	case 'add':
		...
		if ($level0type == "SOUR" || $level0type == "REPO") {
			...
			if ($fact != "OBJE") {
				if ($fact != "ASSO" && $fact != "SOUR" && $fact != "REPO" && $fact != "SHARED_NOTE")
					print_add_layer( "ASSO" );
				// allow to add godfather and godmother for CHR fact or best man and bridesmaid  for MARR fact in one window
				if ($fact == "CHR" || $fact == "MARR")
					print_add_layer( "ASSO2" );
				if ($fact != "SOUR" && $fact != "REPO")
					print_add_layer( "SOUR" );
				if ($fact != "NOTE" && $fact != "SHARED_NOTE")
					print_add_layer( "NOTE" );
				// Shared Note addition ------------
				if ($fact != "SHARED_NOTE" && $fact != "NOTE")
					print_add_layer( "SHARED_NOTE" );
				if ($fact != "REPO")
					print_add_layer( "OBJE" );
			}
	...
	case 'linkfamaction':
		...
		$famrec = ( !isset( $pgv_changes[$famid . "_" . PGV_GEDCOM] ) ) ? find_gedcom_record( $famid, PGV_GED_ID ) : find_updated_record( $famid, PGV_GED_ID );
		$famrec = trim( $famrec );
		if (!empty( $famrec )) {
			$itag = "FAMC";
			...
			if (strpos( $gedrec, "1 $itag @$famid@" ) === false) {
				...
				if ($itag == "FAMC") {
					...
					$$srec = (!isset($pgv_changes[$spid."_".PGV_GEDCOM])) 
						? find_gedcom_record($spid, PGV_GED_ID)
						: find_updated_record($spid, PGV_GED_ID);
			...
			//-- if it is adding a husband or wife
				...
				if ($ct > 0) {
					...
					if ($spid != $pid) {
						...
						if (!empty( $spid )) {
							$srec = ( !isset( $pgv_changes[$spid . "_" . PGV_GEDCOM] ) ) ? find_gedcom_record( $spid, PGV_GED_ID ) : find_updated_record( $spid, PGV_GED_ID );
							if ($srec) {
	...
	<?php
			...
			echo PGV_JS_END;
			?>
	<b><?php echo $pgv_lang['create_source'];
			$tabkey = 1; ?></b>
	<form method="post" action="edit_interface.php" onsubmit="return check_form(this);">
		<input type="hidden" name="action" value="addsourceaction" />
		<input type="hidden" name="pid" value="newsour" />
		<table class="facts_table">
			<tr><td class="descriptionbox <?php echo $TEXT_DIRECTION; ?> wrap width25"><?php print_help_link( "edit_ABBR_help", "qm", "ABBR" );
				echo $factarray["ABBR"]; ?></td>
			...
			<?php $tabkey++; ?>
			<tr><td class="descriptionbox <?php echo $TEXT_DIRECTION; ?> wrap width25"><?php print_help_link( "edit_TITL_help", "qm", "TITL" );
				echo $factarray["TITL"]; ?></td>
			...
			<tr><td class="descriptionbox <?php echo $TEXT_DIRECTION; ?> wrap width25"><?php print_help_link( "edit__HEB_help", "qm", "_HEB" );
				echo $factarray["_HEB"]; ?></td>
			...
			<tr><td class="descriptionbox <?php echo $TEXT_DIRECTION; ?> wrap width25"><?php print_help_link( "edit_ROMN_help", "qm", "ROMN" );
				echo $factarray["ROMN"]; ?></td>
			...
			<?php $tabkey++; ?>
			<?php } ?>
			<tr><td class="descriptionbox <?php echo $TEXT_DIRECTION; ?> wrap width25"><?php print_help_link( "edit_AUTH_help", "qm", "AUTH" );
				echo $factarray["AUTH"]; ?></td>
			...
			<?php $tabkey++; ?>
			<tr><td class="descriptionbox <?php echo $TEXT_DIRECTION; ?> wrap width25"><?php print_help_link( "edit_PUBL_help", "qm", "PUBL" );
				echo $factarray["PUBL"]; ?></td>
			...
			<?php $tabkey++; ?>
			<tr><td class="descriptionbox <?php echo $TEXT_DIRECTION; ?> wrap width25"><?php print_help_link( "edit_REPO_help", "qm", "edit_repo" );
				echo $factarray["REPO"]; ?></td>
			<td class="optionbox wrap"><input tabindex="<?php echo $tabkey; ?>" type="text" name="REPO" id="REPO" value="" size="10" /> <?php print_findrepository_link( "REPO" );
			   print_addnewrepository_link( "REPO" ); ?></td></tr>
			<?php $tabkey++; ?>
			<tr><td class="descriptionbox <?php echo $TEXT_DIRECTION; ?> wrap width25"><?php print_help_link( "edit_CALN_help", "qm", "CALN" );
				echo $factarray["CALN"]; ?></td>
		...
		?>
			...
			<table class="facts_table">
			<tr>
				<td class="descriptionbox <?php echo $TEXT_DIRECTION; ?> wrap width25"><?php print_help_link( "edit_SOUR_EVEN_help", "qm", "source_events" );
					echo $pgv_lang['select_events']; ?></td>
	...
	<?php
			...
			echo PGV_JS_END;
			?>
	<b><?php echo $pgv_lang['create_shared_note'];
			$tabkey = 1; ?></b>
	<form method="post" action="edit_interface.php" onsubmit="return check_form(this);">
	...
	<div class="center font11" style="width:100%;">
		<b><?php echo $pgv_lang["create_shared_note_assisted"];
		$tabkey = 1; ?></b>
		<form method="post" action="edit_interface.php" onsubmit="return check_form(this);">
	...
	case 'addmedia_links':
		...
		function check_form(frm) {
			if (frm.TITL.value=="") {
		$gedrec = (!isset($pgv_changes[$pid."_".PGV_GEDCOM])) ? find_gedcom_record($pid, PGV_GED_ID) : find_updated_record($pid, PGV_GED_ID);return true;
		$note_content = (preg_match("/^0 @$pid@ NOTE ?(.*)/", $gedrec, $n1match)) ? $n1match[1].get_cont(1, $gedrec, false) : '';?>
	<!-- <form method="post" action="edit_interface.php" onsubmit="return check_form(this);"> -->
	...
	<?php
			...
			echo PGV_JS_END;
			?>
	<b><?php echo $pgv_lang['edit_shared_note'];
			$tabkey = 1;
			echo "&nbsp;&nbsp;(" . $pid . ")"; ?></b><br /><br />
	<form method="post" action="edit_interface.php" onsubmit="return check_form(this);">
		...
		<table class="facts_table">
			<tr>
				<td class="descriptionbox <?php echo $TEXT_DIRECTION; ?> wrap width25"><?php print_help_link( "edit_SHARED_NOTE_help", "qm", "shared_note" );
...
//-- add new repository
	...
	<b><?php
			...
			echo $pgv_lang["create_repository"];
			$tabkey = 1;
			?></b>
	<form method="post" action="edit_interface.php" onsubmit="return check_form(this);">
		...
		<table class="facts_table">
			<tr><td class="descriptionbox <?php echo $TEXT_DIRECTION; ?> wrap width25"><?php print_help_link( "edit_REPO_NAME_help", "qm", "REPO" );
				echo $factarray["NAME"]; ?></td>
			...
			<?php $tabkey++; ?>
			<?php } ?>
			<tr><td class="descriptionbox <?php echo $TEXT_DIRECTION; ?> wrap width25"><?php print_help_link( "edit_ADDR_help", "qm", "ADDR" );
				echo $factarray["ADDR"]; ?></td>
			...
			<?php $tabkey++; ?>
			<tr><td class="descriptionbox <?php echo $TEXT_DIRECTION; ?> wrap width25"><?php print_help_link( "edit_PHON_help", "qm", "PHON" );
				echo $factarray["PHON"]; ?></td>
			<td class="optionbox wrap"><input tabindex="<?php echo $tabkey; ?>" type="text" name="PHON" id="PHON" value="" size="40" maxlength="255" /> </td></tr>
			<?php $tabkey++; ?>
			<tr><td class="descriptionbox <?php echo $TEXT_DIRECTION; ?> wrap width25"><?php print_help_link( "edit_FAX_help", "qm", "FAX" );
				echo $factarray["FAX"]; ?></td>
			<td class="optionbox wrap"><input tabindex="<?php echo $tabkey; ?>" type="text" name="FAX" id="FAX" value="" size="40" /></td></tr>
			<?php $tabkey++; ?>
			<tr><td class="descriptionbox <?php echo $TEXT_DIRECTION; ?> wrap width25"><?php print_help_link( "edit_EMAIL_help", "qm", "emailadress" );
				echo $factarray["EMAIL"]; ?></td>
			<td class="optionbox wrap"><input tabindex="<?php echo $tabkey; ?>" type="text" name="EMAIL" id="EMAIL" value="" size="40" maxlength="255" /></td></tr>
			<?php $tabkey++; ?>
			<tr><td class="descriptionbox <?php echo $TEXT_DIRECTION; ?> wrap width25"><?php print_help_link( "edit_WWW_help", "qm", "WWW" );
				echo $factarray["WWW"]; ?></td>
...
//-- get the new incoming raw gedcom record and store it in the file
	case 'updateraw':
		if (isset( $_REQUEST['newgedrec'] ))
			$newgedrec = $_REQUEST['newgedrec'];
		if (PGV_DEBUG) {
			phpinfo( INFO_VARIABLES );
			echo "<pre>$newgedrec</pre>";
		}
...
//			$cens_pids = $cens_pids;
		...
		foreach ( $cens_pids as $pid ) {
			...
			if ($text[0] == "Y" or $text[0] == "y")
				$text[0] = "";
			if (in_array( $tag[0], $emptyfacts ) && array_unique( $text ) == [ "" ] && !$islink[0])
				$text[0] = "Y";
			...
			if (!is_array( $linenum )) {
				...
				if (!empty( $NOTE )) {
					$newlines = preg_split( "/\r?\n/", $NOTE, -1 );
					for ( $k = 0; $k < count( $newlines ); $k++ ) {
						$gedlines[$k] = ( $k == 0 && count( $newlines ) > 1 ) ? "0 @$pid@ NOTE $newlines[$k]\n" : " 1 CONT $newlines[$k]\n";
					}
				}
	...
	case 'addchildaction':
		...
		$gedrec = ( safe_POST_bool( 'SOUR_INDI' ) ) ? handle_updates( $gedrec ) : updateRest( $gedrec );
	...
	if (PGV_DEBUG) {
		...
		if ($famid == "new") {
			...
			$famrec = ( safe_POST_bool( 'SOUR_FAM' ) ) ? handle_updates( $famrec ) : updateRest( $famrec );
			...
			$famrec = "";
			$famrec = isset( $pgv_changes[$famid . "_" . PGV_GEDCOM] ) ? find_updated_record( $famid, PGV_GED_ID ) : find_family_record( $famid, PGV_GED_ID );
			if (!empty( $famrec )) {
				$famrec = trim( $famrec ) . "\n1 $famtag @$xref@\n";
		...
		if (!empty( $pid )) {
			$indirec = "";
			$indirec = ( !isset( $pgv_changes[$pid . "_" . PGV_GEDCOM] ) ) ? find_gedcom_record( $pid, PGV_GED_ID ) : find_updated_record( $pid, PGV_GED_ID );
			if ($indirec) {
	...
	case 'linkspouseaction':
		...
		if (isset( $_REQUEST['spid'] ))
			$spid = $_REQUEST['spid'];
		if (!empty( $spid )) {
			$gedrec = isset( $pgv_changes[$spid . '_' . PGV_GEDCOM] ) ? find_updated_record( $spid, PGV_GED_ID ) : find_person_record( $spid, PGV_GED_ID );
			$gedrec = trim( $gedrec );
			if (!empty( $gedrec )) {
				if ($famid == "new") {
					$famrec = "0 @new@ FAM\n";
...
// ---------------------------------------------------------------------------------
	...
	case 'deleterepo':
		...
		if (!empty( $gedrec )) {
			...
			foreach ( fetch_all_links( $pid, PGV_GED_ID ) as $xref ) {
				$gedrec = isset( $pgv_changes[$xref . '_' . PGV_GEDCOM] ) ? find_updated_record( $xref, PGV_GED_ID ) : find_gedcom_record( $xref, PGV_GED_ID );
				$lines = explode( "\n", $gedrec );
				$newrec = "";
				$skipline = false;
				$glevel = 0;
	...
	case 'al_reset_media_update': // Reset sort using Album Page
		...
		if ($success)
			echo "<br />", $pgv_lang["update_successful"], "<br /><br />";
		$tabno = ( !file_exists( PGV_ROOT . 'modules/googlemap/defaultconfig.php' ) ) ? "7" : "8";
		echo PGV_JS_START;
	...
	<form name="reorder_form" method="post" action="edit_interface.php">
		...
		foreach ( $family->getChildren() as $k => $child ) {
			...
			$sortkey = ( $bdate->isOK() ) ? $bdate->JD() : 1e8;
	...
	case 'changefamily':
		...
		if (count( $children ) > 0) {
			if (!is_null( $father )) {
				if ($father->getSex() == "F")
					$father->setLabel( $pgv_lang["mother"] );
				else
					$father->setLabel( $pgv_lang["father"] );
			}
			if (!is_null( $mother )) {
				if ($mother->getSex() == "M")
					$mother->setLabel( $pgv_lang["father"] );
				else
					$mother->setLabel( $pgv_lang["mother"] );
			}
	...
	case 'changefamily_update':
		...
		if (!empty( $HUSB ) && ( is_null( $father ) || $father->getXref() != $HUSB )) {
			if (strstr( $gedrec, "1 HUSB" ) !== false)
				...
				$gedrec .= "\n1 HUSB @$HUSB@\n";
			$indirec = isset( $pgv_changes[$HUSB . "_" . PGV_GEDCOM] ) ? find_updated_record( $HUSB, PGV_GED_ID ) : find_person_record( $HUSB, PGV_GED_ID );
		...
		if (!is_null( $father ) && $father->getXref() != $HUSB) {
			$indirec = isset( $pgv_changes[$father->getXref() . "_" . PGV_GEDCOM] ) ? find_updated_record( $father->getXref(), PGV_GED_ID ) : find_person_record( $father->getXref(), PGV_GED_ID );
		...
		if (!empty( $WIFE ) && ( is_null( $mother ) || $mother->getXref() != $WIFE )) {
			if (strstr( $gedrec, "1 WIFE" ) !== false)
				...
				$gedrec .= "\n1 WIFE @$WIFE@\n";
			$indirec = isset( $pgv_changes[$WIFE . "_" . PGV_GEDCOM] ) ? find_updated_record( $WIFE, PGV_GED_ID ) : find_person_record( $WIFE, PGV_GED_ID );
			if (!empty( $indirec ) && ( strpos( $indirec, "1 FAMS @$famid@" ) === false )) {
				$indirec .= "\n1 FAMS @$famid@\n";
		...
		if (!is_null( $mother ) && $mother->getXref() != $WIFE) {
			$indirec = isset( $pgv_changes[$mother->getXref() . "_" . PGV_GEDCOM] ) ? find_updated_record( $mother->getXref(), PGV_GED_ID ) : find_person_record( $mother->getXref(), PGV_GED_ID );
			$pos1 = strpos( $indirec, "1 FAMS @$famid@" );
			if ($pos1 !== false) {
				$pos2 = strpos( $indirec, "\n1", $pos1 + 5 );
				if ($pos2 === false)
		...
		while ( isset( $_REQUEST[$var] ) ) {
			...
			if (!empty( $CHIL )) {
				...
				if (strpos( $gedrec, "1 CHIL @$CHIL@" ) === false) {
					$gedrec .= "\n1 CHIL @$CHIL@\n";
					$updated = true;
					$indirec = isset( $pgv_changes[$CHIL . "_" . PGV_GEDCOM] ) ? find_updated_record( $CHIL, PGV_GED_ID ) : find_person_record( $CHIL, PGV_GED_ID );
		...
		foreach ( $children as $key => $child ) {
			if (!is_null( $child )) {
				if (!in_array( $child->getXref(), $newchildren )) {
					...
					$indirec = isset( $pgv_changes[$child->getXref() . "_" . PGV_GEDCOM] ) ? find_updated_record( $child->getXref(), PGV_GED_ID ) : find_person_record( $child->getXref(), PGV_GED_ID );
...
//-- edit a fact record in a form
	...
	<form name="reorder_form" method="post" action="edit_interface.php">
		...
		if (( !empty( $option ) ) && ( $option == "bymarriage" )) {
			uasort( $fams, [ 'Family', 'CompareMarrDate' ] );
		}
		$i = 0;
