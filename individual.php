...
<table border="0" cellspacing="0" cellpadding="0" class="facts_table">
	...
	<td valign="top">
		...
		<?php if (strlen($controller->indi->getAddName() ?? 0) > 0) echo "<span class=\"name_head\">", PrintReady($controller->indi->getAddName()), "</span><br />"; ?>
...
<?php
	...
	$inav = ( $controller->default_tab == 5 ) ? new TreeNav( $controller->pid, 'treetab' ) : new TreeNav( 'none', 'treetab' );
	$inav->generations = 7;
	$inav->zoomLevel = -1;
...
if(empty($SEARCH_SPIDER)) {
	...
	if (file_exists(PGV_ROOT.'modules/research_assistant/research_assistant.php') && ($SHOW_RESEARCH_ASSISTANT>=PGV_USER_ACCESS_LEVEL)) {
		if (version_compare(PGV_VERSION, '4.1')<0) echo "<script src=\"compat.js\" language\"JavaScript\"></script>";
		echo "<script src=\"modules/research_assistant/research_assistant.js\"></script>";
