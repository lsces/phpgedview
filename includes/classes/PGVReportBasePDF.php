...
require_once EXTERNAL_LIBS_PATH."tcpdf/tcpdf.php";
...
class PGVReportBasePDF extends PGVReportBase {
	...
	*/
	public function setup() {
		...
		$this->pdf = new PGVRPDF($this->orientation, parent::unit, [ $this->pagew, $this->pageh ], self::unicode, $this->charset, self::diskcache);
...
class PGVRPDF extends \TCPDF {
	...
	* @return object|boolean false if not numbered befor | object if already numbered
	*/
	...
	*/
	public function render(&$pdf) {
		...
		*/
		...
		$temptext = str_replace(array("#PAGENUM#", "#PAGETOT#"), [ $pdf->getAliasNumPage(), $pdf->getAliasNbPages() ], $this->text);
		...
		$cX = $this->left == "." ? $pdf->GetX()
			// For static position add margin (also updates X)
			: $pdf->addMarginX($this->left);
...
class PGVRHtmlPDF extends PGVRHtml {
	...
	public function render(&$pdf, $sub = false) {
	...
	*/
	public function render(&$pdf) {
		...
		*/
		...
		$cW = $this->width == 0 or ($this->width > $pdf->getRemainingWidthPDF())
			? $pdf->getRemainingWidthPDF() : $this->width;

		// Save the original margins
		$cM = $pdf->getMargins();
		// Use cell padding to wrap the width
		// Temp Width with cell padding
		$cWT = !is_array($cM['cell']) ? -($cM['cell'] * 2) : -($cM['padding_left'] + $cM['padding_right']);
...
class PGVRFootnotePDF extends PGVRFootnote {
	...
	*/
	public function renderFootnote(&$pdf) {
		...
		$temptext = str_replace(array("#PAGENUM#", "#PAGETOT#"), [ $pdf->getAliasNumPage(), $pdf->getAliasNbPages() ], $this->text);
		$temptext = str_replace(array('�', '�'), [ '<u>', '</u>' ], $temptext);		// underline �title� part of Source item
...
//		$pdf->Write($pdf->getCurrentStyleHeight(), $this->num.". ".$temptext."\n\n"); //@@ indi list of sources
	...
	* @return float $h
	*/
	public function getFootnoteHeight(&$pdf) {
