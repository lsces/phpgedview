...
///////////////////////////////////////////////////////////////////////////////
	...
	static function _getFavorites($isged=true) {
		...
		$userfavs = $isged ? getUserFavorites( $GEDCOM ) : getUserFavorites( PGV_USER_NAME );
		$content = '';
		if (!count( $userfavs )) {
			if ($isged) {
			...
			foreach ( $userfavs as $k => $favorite ) {
				...
				if ($favorite['type'] == 'URL') {
					...
					if (displayDetailsById( $favorite['gid'], $favorite['type'] )) {
						require PHPGEDVIEW_PKG_INDEX_PATH . $GEDCOM . '_conf.php';
	...
	static function gedcomFavorites()
	{
		return self::_getFavorites( true );
	}

	static function userFavorites()
	{
		return self::_getFavorites( false );
	}

	static function totalGedcomFavorites()
	{
		return count( getUserFavorites( $GLOBALS['GEDCOM'] ) );
	}

	static function totalUserFavorites()
	{
		return count( getUserFavorites( PGV_USER_NAME ) );
	}
...
///////////////////////////////////////////////////////////////////////////////
	...
	static function userMessages()
	{
		...
		if (count( $usermessages ) == 0) {
			...
			foreach ( $usermessages as $k => $message ) {
				...
				$t = ( !empty( $message['created'] ) ) ? strtotime( $message['created'] ) : time();
	...
	{
		return count( getUserMessages( PGV_USER_NAME ) );
	}
...
///////////////////////////////////////////////////////////////////////////////
	...
	function totalUserJournal()
	{
		return count( getUserNews( PGV_USER_ID ) );
	}
...
///////////////////////////////////////////////////////////////////////////////
	...
	static function gedcomNews( $params = null )
	{
		...
		$limit = ( isset( $params[0] ) && $params[0] != '' ) ? strtolower( $params[0] ) : 'count';
		$flag = ( isset( $params[1] ) && $params[1] != '' ) ? strtolower( $params[0] ) : 5; // News postings
