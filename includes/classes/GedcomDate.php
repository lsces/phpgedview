...
class CalendarDate {
	...
	public static function CALENDAR_ESCAPE() {
		return '@#DUNKNOWN@';
	}
	public static function NUM_MONTHS() {
		return 12;
	}
	public static function MONTH_TO_NUM($m) {
		static $months = [ '' => 0, 'jan' => 1, 'feb' => 2, 'mar' => 3, 'apr' => 4, 'may' => 5, 'jun' => 6, 'jul' => 7, 'aug' => 8, 'sep' => 9, 'oct' => 10, 'nov' => 11, 'dec' => 12 ];
		if (isset($months[$m])) {
			return $months[$m];
		} else {
			return null;
	...
	public static function NUM_TO_MONTH($n) {
		static $months = [ 0 => '', 1 => 'jan', 2 => 'feb', 3 => 'mar', 4 => 'apr', 5 => 'may', 6 => 'jun', 7 => 'jul', 8 => 'aug', 9 => 'sep', 10 => 'oct', 11 => 'nov', 12 => 'dec' ];
		if (isset($months[$n])) {
			return $months[$n];
		} else {
			return null;
	...
	public static function CAL_START_JD() {
		return 0; // @#DJULIAN@ 01 JAN 4713B.C.
	}
	public static function CAL_END_JD() {
		return 99999999;
	}
	public static function NUM_DAYS_OF_WEEK() {
		return 7;
	}
	public static function DAYS_OF_WEEK($n) {
		static $days = [ 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday' ];
		return $days[$n];
	}
	public static function YMDtoJD($y, $m, $d) {
		return 0;
	}
	public static function JDtoYMD($j) {
		return [ 0, 0, 0 ];
	}
	// Most years are 1 more than the previous, but not always (e.g. 1BC->1AD)
	public static function NextYear($y) {
		return $y+1;
	}
	...
	public function FormatDayZeros() {
		if ($this->d<10)
			return '0'.$this->d;
		else
			return $this->d;
	...
	public function FormatLongWeekday() {
		global $pgv_lang;
	...
	public function FormatShortWeekday() {
		global $pgv_lang;
		$day=$this->DAYS_OF_WEEK($this->minJD % self::NUM_DAYS_OF_WEEK());
	...
	public function FormatISOWeekday() {
		return $this->minJD % 7 + 1;
	}

	public function FormatOrdinalSuffix() {
	...
	public function FormatNumericWeekday() {
		return ($this->minJD + 1) % self::NUM_DAYS_OF_WEEK();
	}

	public function FormatDayOfYear() {
	...
	public function FormatMonthZeros() {
		if ($this->m > 9)
			return $this->m;
		else
			return '0'.$this->m;
	...
	public function FormatLongMonth() {
		global $pgv_lang;
	...
	public function FormatShortMonth() {
		global $pgv_lang;
	...
	public function FormatShortYear() {
		return $this->y;
	}

	public function FormatGedcomDay() {
		if ($this->d==0)
			return '';
		else
	...
	public function FormatGedcomMonth() {
		return strtoupper($this->NUM_TO_MONTH($this->m));
	}

	public function FormatGedcomYear() {
		if ($this->y==0)
			return '';
		else
	...
	public function FormatLongYear() {
		return $this->y;
	}
	...
	public static function NumToRoman($num) {
		static $lookup = [ 1000 => 'M', '900' => 'CM', '500' => 'D', 400 => 'CD', 100 => 'C', 90 => 'XC', 50 => 'L', 40 => 'XL', 10 => 'X', 9 => 'IX', 5 => 'V', 4 => 'IV', 1 => 'I' ];
  	if ($num<1) return $num;
		$roman='';
	...
	public static function RomanToNum($roman) {
		static $lookup = [ 1000 => 'M', '900' => 'CM', '500' => 'D', 400 => 'CD', 100 => 'C', 90 => 'XC', 50 => 'L', 40 => 'XL', 10 => 'X', 9 => 'IX', 5 => 'V', 4 => 'IV', 1 => 'I' ];
		$num=0;
		foreach ($lookup as $key=>$value)
...
class JewishDate extends CalendarDate {
	...
	public static function MONTH_TO_NUM($m) {
		static $months = [ '' => 0, 'tsh' => 1, 'csh' => 2, 'ksl' => 3, 'tvt' => 4, 'shv' => 5, 'adr' => 6, 'ads' => 7, 'nsn' => 8, 'iyr' => 9, 'svn' => 10, 'tmz' => 11, 'aav' => 12, 'ell' => 13 ];
		if (isset($months[$m])) {
			return $months[$m];
		} else {
			return null;
	...
	public static function NUM_TO_MONTH($n) {
		static $months = [ 0 => '', 1 => 'tsh', 2 => 'csh', 3 => 'ksl', 4 => 'tvt', 5 => 'shv', 6 => 'adr', 7 => 'ads', 8 => 'nsn', 9 => 'iyr', 10 => 'svn', 11 => 'tmz', 12 => 'aav', 13 => 'ell' ];
		if (isset($months[$n])) {
			return $months[$n];
		} else {
			return null;
	...
	public static function YMDtoJD($y, $mh, $d) {
...
class HebrewDate extends JewishDate {
	public const string GERSHAYIM="״";
	public const string GERSH="׳";
	public const string ALAFIM="אלפים";
	
	public static $HEBREW_MONTHS = [ "", "תשרי", "חשוון", "כסלו", "טבת", "שבט", "אדר", "אדר ב׳", "ניסן", "אייר", "סיוון", "תמוז", "אב", "אלול" ];
	public static $HEBREW_DAYS   = [ "שני", "שלישי", "רביעי", "חמישי", "ששי", "שבת", "ראשון" ];
	...
	public static function NumToHebrew($num) {
		...
		static $jHundreds = [ "", "ק", "ר", "ש", "ת", "תק", "תר", "תש", "תת", "תתק" ];
		static $jTens     = [ "", "י", "כ", "ל", "מ", "נ", "ס", "ע", "פ", "צ" ];
		static $jTenEnds  = [ "", "י", "ך", "ל", "ם", "ן", "ס", "ע", "ף", "ץ" ];
		static $tavTaz    = [ "ט״ו", "ט״ז" ];
		static $jOnes     = [ "", "א", "ב", "ג", "ד", "ה", "ו", "ז", "ח", "ט" ];
...
class ArabicDate extends HijriDate {
	public static $ARABIC_MONTHS = [ "", "محرّم", "صفر", "ربيع الأول", "ربيع الثانى", "جمادى الأول", "جمادى الثاني", "رجب", "شعبان", "رمضان", "شوّال", "ذو القعدة", "ذو الحجة" ];
	public static $ARABIC_DAYS   = [ "الأثنين", "الثلاثاء", "الأربعاء", "الخميس", "الجمعه", "السبت", "الأحد" ];
...
////////////////////////////////////////////////////////////////////////////////
class GedcomDate {
	public $qual1 = null; // Optional qualifier, such as BEF, FROM, ABT
	public $date1 = null; // The first (or only) date
	public $qual2 = null; // Optional qualifier, such as TO, AND
	public $date2 = null; // Optional second date
	public $text  = null; // Optional text, as included with an INTerpreted date
	...
	public static function ParseDate($date) {
		...
		if ($cal=='') {
			$cal = preg_match('/^(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)$/', $m) #
				? '@#dgregorian@'
				: ( preg_match('/^[345]\d\d\d$/', $y) // Year 3000-5999
				? '@#dhebrew@' : '@#dgregorian@' );
		}
	...
	public function Display($url=false, $date_fmt=null, $cal_fmts=null) {
		...
		if (!function_exists($func))
			$func="\Bitweaver\Phpgedview\DefaultDateLocalisation";

		// Two dates with text before, between and after
		$q1=$this->qual1;
		$d1=$this->date1->Format($date_fmt);
		$q2=$this->qual2;
		$d2= $this->date2 === null ? '' : $this->date2->Format($date_fmt);
		$q3='';
		// Localise the date
		$func($q1, $d1, $q2, $d2, $q3);
		...
		foreach ($cal_fmts as $cal_fmt)
			if ($cal_fmt!='none') {
				...
				$d1tmp = $d1conv->InValidRange() ? $d1conv->Format($date_fmt) : '';
				$q1tmp=$this->qual1;
				if (is_null($this->date2)) {
					...
					$d2tmp = $d2conv->InValidRange() ? $d2conv->Format($date_fmt) : '';
				}
				$q2tmp=$this->qual2;
				$q3tmp='';
				// Localise the date
	...
	public function MinDate(): ArabicDate|FrenchRDate|GregorianDate|HebrewDate|HijriDate|JewishDate|JulianDate|RomanDate {
		return $this->date1;
	}
	public function MaxDate(): ArabicDate|FrenchRDate|GregorianDate|HebrewDate|HijriDate|JewishDate|JulianDate|RomanDate {
		if ( $this->date2 === null )
			return $this->date1;
		else
			return $this->date2;
	}
	public function MinJD() {
		$tmp=$this->MinDate();
		return $tmp->minJD;
	}
	public function MaxJD() {
		$tmp = $this->MaxDate();
		return $tmp->maxJD;
	}
	public function JD() {
	...
	public function AddYears($n, $qual='') {
		$tmp=clone $this;
		$tmp->date1->y+=$n;
		$tmp->date1->m=0;
		$tmp->date1->d=0;
	...
	public static function GetAgeYears($d1, $d2=null, $warn_on_negative=true) {
	...
	public static function GetAgeGedcom($d1, $d2=null, $warn_on_negative=true) {
		if (is_null($d2)) {
