 ...
 * @param string $personRec - The GEDCOM record where the person can be found
 * @param string $personID - The id of the person record
 * @return string GrampsXML for the person
 */
public function create_person($personRec = "", $personID = "") {
		...
		if (!isset($this->dom))
		{
		 ...
		 $this->ePlaces = $eRoot->appendChild($this->ePlaces);
			...
			$etGender = ( ( $gender = get_gedcom_value( "SEX", 1, $personRec ) ) != null ) ? $this->dom->createTextNode( $gender ) : $this->dom->createTextNode( "U" );
...
if (($nameRec = get_sub_record(1, "1 NAME", $personRec)) != null) {
	...
	*
	* @param string $indirec - The full INDI GEDCOM record of the person the lds_ord is being created
	* @param string $eventName - the name of the LDS event (Baptism, Sealing, Endowment, etc...)
	* @param string $eventABV - the event abbreviation in the GEDCOM (ie. SLGC, BAPL, ENDL)
	* @param string $eParent - The parent element the lds event is attached to
	*/
	public function create_lds_event($indirec, $eventName, $eventABV, $eParent) {
 ...
 * @param string $sourceRec - The GEDCOM record where the source is found
 * @param string $sourceID - The ID of the source
 * @param int $level - the level the source is on, the default is one
 * @return string GrampsXML for source
 */
public function create_source($sourceRec, $sourceID, $level = 1,$done=1) {
	global $eRoot;
			if (!isset($this->dom))
		{
	     $this->dom = new \DomDocument("1.0", "UTF-8");
 ...
 * @param string $frec - The GEDCOM record where the family can be found
 * @param string $fid - The id of the family record
 * @return string GrampsXML for family
 */
public function create_family($frec, $fid) {
	global $eRoot;
			if (!isset($this->dom))
		{
	     $this->dom = new \DomDocument("1.0", "UTF-8");
 ...
 * @param string $fid - The id of the record
 * @return string GrampsXML for the recrord
 */
public function create_record($fid)
{
 ...
 * @param string $mediaRec - The GEDCOM record of the media
 * @param string $mediaID - The id of the media
 * @param int $level - The level on which the media can be found, the default is 1
 * @return string GrampsXML for the recrord
 */
public function create_media($mediaRec, $mediaID, $level = 1)
{
		global $file, $eRoot;
		if (!isset($this->dom))
		{
	...
	public function create_mediaref($eParent, $sourcerefRec, $level,$done=1) {
	...
	public function create_sourceref($eParent, $sourcerefRec, $level,$done=1) {
