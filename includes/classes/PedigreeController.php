...
class PedigreeControllerRoot extends BaseController {
	...
	public function init() {
		...
		if (!$this->show_full) {
			if ($this->talloffset==0) {
				...
				$baseyoffset = ( $this->PEDIGREE_GENERATIONS == 3 ) ? 30 : -85;
			}
		} else {
			if ($this->talloffset==0) {
		...
		for($i= $this->treesize - 1; $i>=0; $i--) {
			...
			if ($this->talloffset==0) {
				...
				if ($this->curgen<$this->PEDIGREE_GENERATIONS) {
					$parent = floor(($i-1)/2);
					$this->yoffset = ( $i % 2 == 0 ) ? $this->yoffset - ( $boxspacing / 2 ) * ( $this->curgen - 1 ) : $this->yoffset + ( $boxspacing / 2 ) * ( $this->curgen - 1 );
					$pgen = $this->curgen;
					while($parent>0) {
						$this->yoffset = ( $parent % 2 == 0 ) ? $this->yoffset - ( $boxspacing / 2 ) * $pgen : $this->yoffset + ( $boxspacing / 2 ) * $pgen;
						$pgen++;
						if ($pgen>3) {
							$temp=0;
							for($j=1; $j<($pgen-2); $j++) $temp += pow( 2, $j ) - 1;
							$this->yoffset = $parent % 2 == 0 ? $this->yoffset - ( $boxspacing / 2 ) * $temp : $this->yoffset + ( $boxspacing / 2 ) * $temp;
						}
					...
					if ($this->curgen>3) {
						...
						$this->yoffset = ( $i % 2 == 0 ) ? $this->yoffset - ( $boxspacing / 2 ) * $temp : $this->yoffset + ( $boxspacing / 2 ) * $temp;
					}
				...
				$this->xoffset = $this->show_full ? $this->curgen * ( ( $this->pbwidth + $bxspacing ) / 2 ) + $this->curgen * 10 + 136.5 : $this->curgen * ( ( $this->pbwidth + $bxspacing ) / 4 ) + $this->curgen * 10 + 215.75;
				if ($this->isPrintPreview()) $this->xoffset -= 260;
			}
			else {
				$this->xoffset = $this->show_full ? ( $this->PEDIGREE_GENERATIONS - $this->curgen ) * ( ( $this->pbwidth + $bxspacing ) / 2 ) + 260 : ( $this->PEDIGREE_GENERATIONS - $this->curgen ) * ( ( $this->pbwidth + $bxspacing ) / 4 ) + 270;
				if ($this->isPrintPreview()) $this->xoffset -= 260;
			}
	...
	public function adjust_subtree($index, $diff) {
		...
		if ($f<count($treeid)) $this->adjust_subtree($f, $diff);
		if ($m<count($treeid)) $this->adjust_subtree($m, $diff);
	}

	public function collapse_tree($index, $curgen, $diff) {
		...
		$odiff=$diff;
		$fdiff = $this->collapse_tree($f, $curgen+1, $diff);
		...
		//print "UP $index BY $diff<br />\n";
		$mdiff = $this->collapse_tree($m, $curgen+1, $diff);
		$zdiff = $mdiff - $fdiff;
		if (($zdiff>0)&&($curgen<$this->PEDIGREE_GENERATIONS-2)) {
			$offsetarray[$index]["y"] -= $boxspacing*$zdiff/2;
			//print "UP $index BY ".($zdiff/2)."<br />\n";
			if ((empty($treeid[$m]))&&(!empty($treeid[$f]))) $this->adjust_subtree($f, -1*($boxspacing*$zdiff/4));
			$diff+= $zdiff / 2;
		}
		return $diff;
	}
