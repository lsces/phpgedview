...
class DescendancyControllerRoot extends BaseController {
	...
	$this->show_full  =safe_GET('show_full', [ '0', '1' ], $PEDIGREE_FULL_DETAILS);
	...
	$this->cellwidth = empty($this->name) || strlen($this->name) < 30
		? $this->cellwidth="420" : strlen($this->name) * 14;
	...
	*/
	 ...
	 */
	public function print_child_family(&$person, $depth, $label="1.", $gpid="") {
		global $personcount;
 ...
 * @param int $depth the descendancy depth to show
 */
public function print_child_descendancy(&$person, $depth) {
	global $pgv_lang;
	...
	print_pedigree_person($person->getXref(), 1, $this->view!="preview",0,$personcount);
	print "</td>";
 ...
 * @param int $depth the descendancy depth to show
 */
public function print_family_descendancy(&$person, &$family, $depth) {
