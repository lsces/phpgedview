...
class MediaControllerRoot extends IndividualController{
	...
	*/
	public function &getEditMenu() {
		...
		if (PGV_USER_CAN_EDIT) {
			...
			$submenu = ( PGV_USER_GEDCOM_ADMIN && file_exists( PGV_ROOT . 'modules/GEDFact_assistant/_MEDIA/media_1_ctrl.php' ) )
				? new Menu( $pgv_lang["add_or_remove_links"] )
				: new Menu( $pgv_lang["set_link"] );
	...
	* @return boolean
	*/
	public function canShowOtherMenu() {
	...
	* @return boolean
	*/
	public function canShowGedcomRecord() {
		global $SHOW_GEDCOM_RECORD;
	...
	* @return array
	*/
	public function getFacts($includeFileName=true) {
...
//		if ($includeFileName) $facts[] = new Event("1 FILE ".$this->mediaobject->getFilename());
		...
		$facts[] = isset( $pgv_lang["TYPE__$mediaType"] ) ? new Event( "1 TYPE " . $pgv_lang["TYPE__$mediaType"] ) : new Event( "1 TYPE " . $pgv_lang["TYPE__other"] );
		...
		if (isset($pgv_changes["{$this->pid}_$GEDCOM"]) && $this->show_changes) {
			...
			$newfacts[] = isset( $pgv_lang["TYPE__$mediaType"] ) ? new Event( "1 TYPE $mediaType" ) : new Event( "1 TYPE " . $pgv_lang["TYPE__other"] );
