...
class Person extends GedcomRecord {
	...
	private $globalfacts = [];
	private $mediafacts = [];
	private $facts_parsed = false;
	private $fams = null;
	private $famc = null;
	private $spouseFamilies = null;
	private $childFamilies = null;
	private $label = '';
	private $highlightedimage = null;
	private $weblinks = null;
	private $file = '';
	private $age = null;
	private $isdead = -1;
	private $sex=null;
	public $generation; // used in some lists to keep track of this Person's generation in that list
	...
	public function __construct($data, $simple=true) {
		if (is_array($data)) {
	...
	public static function CompareBirtDate($x, $y) {
	...
	public static function CompareDeatDate($x, $y) {
	...
	* @return array
	*/
	public function findWebLinkMedia() {
	...
	* @return string
	*/
	public function getCensBirthPlace() {
		global $pgv_lang;
		if (is_null($this->_getBirthPlace)) {
			if ($this->canDisplayDetails()) {
				foreach ($this->getAllBirthPlaces() as $place) {
	...
	*/
	public function getDeathDate($estimate = true) {
		global $pgv_lang;
	...
	* @return string
	*/
	public function getBirthDeathYears($age_at_death=true, $classname='details1') {
		global $pgv_lang;
		...
		if (is_null($this->_getEstimatedDeathDate)) {
			...
			if (is_null($this->_getEstimatedDeathDate)) {
				...
				if ($tmp->MinJD()) {
					...
					$this->_getEstimatedDeathDate = $tmp2->MaxJD()<server_jd() ? $tmp2 : new GedcomDate(''); // always return a date object
				} else {
	...
	*/
	public function getSex() {
		if ( $this->sex === null ) {
			$this->sex = preg_match('/\n1 SEX ([MF])/', $this->gedrec, $match) ? $match[1] : 'U';
		}
		return $this->sex;
	}
	...
	* @return string  <img ... />
	*/
	public function getSexImage($size='small', $style='', $title='') {
		return self::sexImage($this->getSex(), $size, $style, $title);
	}

	public static function sexImage($sex, $size='small', $style='', $title='') {
	...
	* @return string
	*/
	public function getLabel($elderdate='', $counter=0) {
		...
		if (is_object($elderdate) && $elderdate->isOK()) {
			...
			if ($p2->isOK()) {
				...
				if (!function_exists($func))
					$func='\Bitweaver\Phpgedview\DefaultGetLabel';
	...
	*/
	public function getSpouseFamilyIds() {
	...
	* @return Person|null  this person's spouse
	*/
	...
	*/
	public function getChildFamilyIds() {
		if ( $this->famc === null ) {
	...
	*/
	public function getChildFamilyPedigree($famid) {
		$subrec = get_sub_record(1, '1 FAMC @'.$famid.'@', $this->gedrec);
		$pedi = get_gedcom_value('PEDI', 2, $subrec, 0, false);
	...
	* @return array array of Family objects
	*/
	public function getStepFamilies() {
		$families = [];
	...
	* @return array
	*/
	public function getGlobalFacts() {
		$this->parseFacts();
	...
	* @return array
	*/
	public function getIndiFacts($nfacts=null) {
	...
	* @return string
	*/
	public function getChildFamilyLabel(&$family) {
		global $pgv_lang;
		if (!is_null($family)) {
	...
	* @return string
	*/
	public function getStepFamilyLabel(&$family) {
		global $pgv_lang;
		...
		foreach ($childfams as $fam) {
			if (!$fam->equals($family)) {
				$wife = $fam->getWife();
				$husb = $fam->getHusband();
				if ( ( $husb === null || !$husb->equals($father) ) && ( $wife === null || $wife->equals($mother) ) ) {
					$label = $mother->getSex()=='M' ? $pgv_lang['fathers_family_with'] : $pgv_lang['mothers_family_with'];
					if ( !$father === null ) $label .= $father->getFullName();
					else $label .= $pgv_lang['unknown'];
				}
				else if ((is_null($wife) || !$wife->equals($mother)) && (is_null($husb)||$husb->equals($father))) {
					$label = $father->getSex()=='F' ? $pgv_lang['mothers_family_with'] : $pgv_lang['fathers_family_with'];
					if (!$mother === null) $label .= $mother->getFullName();
					else $label .= $pgv_lang['unknown'];
				}
	...
	* @return string
	*/
	public function getSpouseFamilyLabel(&$family) {
	...
	*/
	public function parseFacts($nfacts=null) {
		...
		if (!$sexfound) {
			$this->globalfacts[] = new Event('1 SEX U', 0);
		}
	}
	/**
	...
	*/
	public function add_family_facts($otherfacts = true) {
		global $GEDCOM, $nonfacts, $nonfamfacts;

		if (!isset($nonfacts)) $nonfacts = [];
		if (!isset($nonfamfacts)) $nonfamfacts = [];
	...
	* @return void records added to indifacts array
	*/
	public function add_parents_facts(&$person, $sosa=1) {
		...
		/* @var $family Family */
		foreach ($fams as $famid=>$family) {
			foreach ( [ $family->getWife(), $family->getHusband() ] as $parent) {
				if ($parent) {
					// add parent death
					if ($sosa==1) {
				...
				if (strstr($SHOW_RELATIVES_EVENTS, $fact)) {
					...
					foreach ($sfamids as $sfamid=>$sfamily) {
						...
						if (GedcomDate::Compare($bDate, $sEvent->getDate())<0 && GedcomDate::Compare($sEvent->getDate(), $dDate)<=0) {
							...
							$factrec .= "\n2 ASSO @".$parent->getXref().'@';
							$factrec .= "\n3 RELA *".$rela;
							$rela2 = $rela == 'father' ? 'stepmom' : 'stepdad';
	...
	* @return void records added to indifacts array
	*/
	public function add_children_facts(&$family, $option='_CHIL', $except='') {
		global $SHOW_RELATIVES_EVENTS, $factarray;
	...
	* ddate = indi death date record
	*
	* @param Person $spouse Person object
	* @param string $famrec family Gedcom record
	* @return void records added to indifacts array
	*/
	public function add_spouse_facts(&$spouse, $famrec='') {
		global $SHOW_RELATIVES_EVENTS, $factarray;
	...
	* @return void records added to indifacts array
	*/
	public function add_stepsiblings_facts(&$spouse, $except='') {
	...
	*
	*/
	public function add_historical_facts() {
		...
		if ($SHOW_RELATIVES_EVENTS && file_exists(PGV_ROOT.'languages/histo.'.$lang_short_cut[$LANGUAGE].'.php')) {
			...
			foreach ($histo as $indexval=>$hrec) {
				$sdate=new GedcomDate(get_gedcom_value('DATE', 2, $hrec, 0, false));
				if ($sdate->isOK() && GedcomDate::Compare($this->getEstimatedBirthDate(), $sdate)<=0 && GedcomDate::Compare($sdate, $this->getEstimatedDeathDate())<=0) {
	...
	*
	*/
	public function add_asso_facts() {
		global $factarray, $pgv_lang;
	...
	protected function _addName($type, $full, $gedrec) {
		...
		if (strpos($givn, '@P.N.')!==false || $surn=='@N.N.' || $surns[0]=='@N.N.') {
			if (strpos($givn, '@P.N.')!==false && ($surn=='@N.N.' || $surns[0]=='@N.N.')) {
				...
				$PN = $surn!=='' ? $unknownPN[whatLanguage($surn)] : $unknownPN[whatLanguage($surns[0])];
				$NN=$unknownNN[whatLanguage($givn)];
			}
			$list=str_replace( [ '@N.N.', '@P.N.' ], [ $NN, $PN ], $list);
			$full=str_replace( [ '@N.N.', '@P.N.' ], [ $NN, $PN ], $full);
		}
