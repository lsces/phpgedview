...
//	var $soundex = "Russell";
	...
	function SoundexSearch() {
		...
		usort($this->myindilist, array('\Bitweaver\Phpgedview\GedcomRecord', 'Compare'));
		usort($this->myfamlist, array('\Bitweaver\Phpgedview\GedcomRecord', 'Compare'));
	}
			...
			if ($this->myindilist || $this->myfamlist || $this->mysourcelist || $this->mynotelist) {
				...
				foreach ($this->sgeds as $ged_id=>$gedcom) {
					...
					if ($datalist) {
						...
						usort($datalist, array('\Bitweaver\Phpgedview\GedcomRecord', 'Compare'));
						$GEDCOM=$gedcom;
