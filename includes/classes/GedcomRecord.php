...
class GedcomRecord {
	...
	public static function getInstance($data) {
		...
		switch($type) {
		case 'INDI':
			$class_name='\Bitweaver\Phpgedview\Person';
			break;
		case 'FAM':
			$class_name='\Bitweaver\Phpgedview\Family';
			break;
		case 'SOUR':
			$class_name='\Bitweaver\Phpgedview\Source';
			break;
		case 'OBJE':
			$class_name='\Bitweaver\Phpgedview\Media';
			break;
		case 'REPO':
			$class_name='\Bitweaver\Phpgedview\Repository';
			break;
		case 'NOTE':
			$class_name='\Bitweaver\Phpgedview\Note';
			break;
		default:
			$class_name='\Bitweaver\Phpgedview\GedcomRecord';
			break;
		}
	...
	*/
	...
	protected function _getLinkUrl($link) {
		$url = $this->ged_id
			...
			? $link.$this->getXref().'&ged='.get_gedcom_from_id($this->ged_id)
			// If the record was created from a text string, assume the current gedcom
			: $link.$this->getXref().'&ged='.PGV_GEDCOM;
		if ($this->isRemote()) {
	...
	*/
	...
	protected function _addName($type, $value, $gedrec) {
		$this->_getAllNames[]= [
			...
			'sort' => preg_replace_callback( '/([0-9]+)/',
				fn( $match ) => substr( "000000000$match[1]", -10 ),
				$value ),
		];
	}
	...
	public function getSecondaryName() {
		if ( $this->_getSecondaryName === null ) {
	...
	public static function CompareChanDate($x, $y) {
		...
		if ($tmp) {
			...
			if (
				preg_match('/^\d\d:\d\d:\d\d/', get_gedcom_value('DATE:TIME', 2, $chan_x->getGedcomRecord(), 0, false).':00', $match_x) &&
				preg_match('/^\d\d:\d\d:\d\d/', get_gedcom_value('DATE:TIME', 2, $chan_y->getGedcomRecord(), 0, false).':00', $match_y)
			) {
	...
	public function format_list($tag='li', $find=false, $name=null) {
		...
		$href = $find
			? 'javascript:;" onclick="pasteid(\''.$this->getXref().'\', \''.htmlspecialchars(strip_tags($this->getFullName()), ENT_QUOTES).'\'); return false;'
	...
	*/
	public function parseFacts($nfacts=null) {
		...
		if (!$this->facts === null && is_array($this->facts)) {
			return;
		}
		$this->facts=[];
	...
	*/
	...
	/**
	 * Summary of getAllEvents
	 * @param mixed $event
	 * @return string
	 * TODO work out just what this is suposed to do
	 */
	public function getAllEvents($event) {
		$srec = [];
		if (!$srec) {
			return '';
		}
	...
	public function getEventSource($event) {
		...
		$srec = $srec[0];
		return get_sub_record( 2,'SOUR', $srec);
	}
	...
	public function LastChangeTimestamp($add_url) {
		...
		$d = $chan->getDate();
		if (preg_match('/^(\d\d):(\d\d):(\d\d)/', get_gedcom_value('DATE:TIME', 2, $chan->getGedcomRecord(), 0, false).':00', $match)) {
			$t=mktime($match[1], $match[2], $match[3]);
			$sort=$d->MinJD().$match[1].$match[2].$match[3];
			$text=strip_tags($d->Display(false, "{$DATE_FORMAT} -", []).date(" {$TIME_FORMAT}", $t));
		} else {
			$sort=$d->MinJD().'000000';
			$text=strip_tags($d->Display(false, "{$DATE_FORMAT}", []));
		}
		if ($add_url) {
