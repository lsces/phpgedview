...
class IndividualControllerRoot extends BaseController {
	...
	public $indi = null;
	public $diffindi = null;
	public $NAME_LINENUM = 1;
	public $accept_success = false;
	...
	public $SEX_COUNT = 0;
	public $sexarray = [];
	public $tabarray = [ "facts", "notes", "sources", "media", "relatives", "tree", "research", "map", "lightbox", "spare", "nav" ];
				...
				if (count($parts)==2) {
					...
					if (!empty($servid)&&!empty($aliaid)) {
						...
						if (!is_null($serviceClient)) {
							$mergerec = !empty( $newrec ) ? $serviceClient->mergeGedcomRecord( $aliaid, $newrec, true ) : $serviceClient->mergeGedcomRecord( $aliaid, $indirec, true );
...
//				echo ADOdb::getQueryLog();
	...
	function getHighlightedObject() {
		...
		if ($this->canShowHighlightedObject()) {
			...
			if (!empty($firstmediarec)) {
				...
				$class = ( !$USE_THUMBS_MAIN || $firstmediarec["_THUM"] == 'Y' ) ? "image" : "thumbnail";
	...
	*/
	public function buildFamilyList(&$family, $type) {
			...
			if ($family->isNotMarried()) {
				...
				if (!empty($marr_rec)) {
					...
					if (empty($type) || stristr($type, "partner")===false) {
						...
						$label = $pgv_lang[$type] ?? $pgv_lang["partner"];
						$labels["parent"] = $label;
						$labels["mother"] = $label;
						$labels["father"] = $label;
					}
		...
		for($i=0; $i<$num; $i++) {
			if (!is_null($children[$i])) {
				...
				$pedi = get_gedcom_value("PEDI", 2, $famcrec, 0, false);
				if ($pedi) {
					if ($pedi=="birth") {
	...
	*/
	function printParentsRows(&$family, &$people, $type) {
		...
		if ($family->getMarriageRecord()!="" || PGV_USER_CAN_EDIT) {
			...
			if (!$date && $this->show_changes && isset($pgv_changes[$famid."_".PGV_GEDCOM])) {
				$famrec = find_updated_record($famid, PGV_GED_ID);
				$marrrec = get_sub_record(1, "1 MARR", $famrec);
				if ($marrrec!=$family->getMarriageRecord()) {
					$date = new GedcomDate(get_gedcom_value("MARR:DATE", 1, $marrrec, 0, false));
					$place = get_gedcom_value("MARR:PLAC", 1, $marrrec, 0, false);
					$styleadd = "blue";
				}
			}
			?>
			<tr>
				...
				<td class="facts_value<?php echo $styleadd ?>">
					...
					if ($date && $date->isOK() || $place) {
						...
						$marr_type = $family->getMarriageType() ? "MARR_" . strtoupper( $family->getMarriageType() ) : "MARR";
						if (isset($factarray[$marr_type])) {
							if (isset($factdetail)) {
	...
	*/
	function printChildrenRows(&$family, &$people, $type) {
		...
		foreach($people["children"] as $key=>$child) {
			...
			$styleadd = ( $label[0] == '+' ) ? "blue" : ( ( $label[0] == '-' ) ? "red" : "" );
			?>
			<tr>
...
// LB Fix for no googlemaps ==========================================================================
		...
		$tab_array = ( file_exists( PGV_ROOT . 'modules/googlemap/defaultconfig.php' ) ) ? [ "facts", "notes", "sources", "media", "relatives", "tree", "research", "map", "lightbox", "spare", "nav" ] : [ "facts", "notes", "sources", "media", "relatives", "tree", "research", "lightbox", "spare", "nav" ];
...
//				$this->print_navigator_tab();
	...
	function print_facts_tab() {
		...
		$Fam_Navigator = isset( $_COOKIE['famnav'] ) ? $_COOKIE['famnav'] : "YES";
		// Hide/Show the Family Navigator on this tab =========
		$Fam_Navigator = ( isset( $NAV_FACTS ) && $NAV_FACTS == "SHOW" ) ? "YES" : "HIDE";
		// ===============================================
	...
	function print_notes_tab() {
		...
		// Use Show or Hide Navigator Cookie -----------
		$Fam_Navigator = $_COOKIE['famnav'] ?? "YES";
		// Hide/Show the Family Navigator on this tab =========
		$Fam_Navigator = ( isset( $NAV_NOTES ) && $NAV_NOTES == "SHOW" ) ? "YES" : "HIDE";
		// ===============================================
	...
	function print_sources_tab() {
		...
		$Fam_Navigator = $_COOKIE['famnav'] ?? "YES";
		// Hide/Show the Family Navigator on this tab =========
		$Fam_Navigator = ( isset( $NAV_SOURCES ) && $NAV_SOURCES == "SHOW" ) ? "YES" : "HIDE";
		// ===============================================
	...
	*/
	function print_media_tab() {
		...
		$Fam_Navigator = $_COOKIE['famnav'] ?? "YES";
		// Hide/Show the Family Navigator on this tab =========
		$Fam_Navigator = ( isset( $NAV_MEDIA ) && $NAV_MEDIA == "SHOW" ) ? "YES" : "HIDE";
		// ===============================================
...
<?php } ?>
	...
	function print_research_tab() {
		global $pgv_lang, $SHOW_RESEARCH_ASSISTANT, $CONTACT_EMAIL, $GEDCOM, $factarray, $templefacts, $nondatefacts, $nonplacfacts;
	...
	*/
	function print_lightbox_tab() {
		...
		$Fam_Navigator = $_COOKIE['famnav'] ?? "YES";
		// Hide/Show the Family Navigator on this tab =========
		$Fam_Navigator = ( isset( $NAV_ALBUM ) && $NAV_ALBUM == "SHOW" ) ? "YES" : "HIDE";
		// ===============================================
