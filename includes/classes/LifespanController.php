...
$nonfacts = [ "FAMS", "FAMC", "MAY", "BLOB", "OBJE", "SEX", "NAME", "SOUR", "NOTE", "BAPL", "ENDL", "SLGC", "SLGS", "_TODO", "_PGV_OBJS", "CHAN", "HUSB", "WIFE", "CHIL", "BIRT", "DEAT", "BURI" ];// DEATH OF SIBLING:  DEATH OF HALF SIBLING DEATH OF MOTHER DEATH OF FATHER DEATH OF CHILD
$nonfamfacts = [ "CHAN", "HUSB", "WIFE", "CHIL" ];
...
class LifespanControllerRoot extends BaseController {
	...
	*/
	public function init() {
		...
		if (safe_GET('clear', '1')=='1') {
			...
			uasort($this->people, "\Bitweaver\Phpgedview\compare_people");
	...
	* @param string $newpid
	*/
	public function addFamily($newpid, $gen=0) {
		if (!empty ($newpid)) {
			$person = Person::getInstance($newpid);
			...
			foreach($families as $famID => $family){
				if($newpid != $family->getHusbId()) {
					$this->pids[] = $gen > 0 ? $this->addFamily( $family->getHusbId(), $gen - 1 ) : $family->getHusbId();
				}
				if($newpid != $family->getWifeId()) {
					$this->pids[] = $gen > 0 ? $this->addFamily( $family->getWifeId(), $gen - 1 ) : $family->getWifeId();
				}
				$children = $family->getChildren();
				foreach($children as $childID => $child){
					$this->pids[] = $gen > 0 ? $this->addFamily( $child->getXref(), $gen - 1 ) : $child->getXref();
				}
			}
	...
	public function fillTL($ar, $int, $top) {
		...
		foreach ($ar as $key => $value) {
			...
			/* @var $value Person */
				...
				$minlength = ( stristr( $value->getFullName(), "starredname" ) )
					? ( UTF8_strlen( $value->getFullName() ) - 34 ) * $this->zoomfactor
					: UTF8_strlen( $value->getFullName() ) * $this->zoomfactor;
