...
class ServiceClient extends GedcomRecord {
	...
	public $type = "";
	public $data_type = "";
	public $client_type = "SOAP"; // whether to use SOAP or PEAR:SOAP by default
	...
	*/
	public function getRemoteRecord($remoteid) {
	...
	*/
	public function _merge($record1, $record2) {
		...
		$pos1 = strpos($record1, "\n1");
		$localrec = $pos1 !== false ? substr( $record1, 0, $pos1 + 1 ) : $record1;
...
//print "[<pre>$localrec</pre>]";
	...
	*/
	public function UpdateFamily($record1,$record2){
	...
	*/
	public function MergeForUpdateFamily($Family1,$Family2,$Familylist,&$FamilyListReturn){
		...
		$famrec1 = isset( $pgv_changes["{$Family1}_$GEDCOM"] )
			? find_updated_record( $Family1, get_id_from_gedcom( $GEDCOM ) )
			: find_family_record( $Family1, get_id_from_gedcom( $GEDCOM ) );
	...
	*/
	public function CompairForUpdateFamily($family1,$family2) {
		global $GEDCOM;
	...
	*/
	public static function setSameId($local, $remote) {
		...
		if (empty($gid)) {
			$gBitDb->query(
				...
				, array($local, $remote, get_id_from_gedcom($GEDCOM) ) );
		}
	}
	...
	* of the person
	*/
	public static function ComparePeople(&$Person1,&$Person2){
	...
	* @return string
	*/
	public function checkIds($gedrec) {
		$ids_checked = array();
	...
	*/
	public function mergeGedcomRecord($xref, $localrec, $isStub=false, $firstLink=false) {
	...
	*/
	public function &search($query, $start=0, $max=100) {
		$this->authenticate();
	...
	*/
	public function __change_encoding(&$wsdl) {
	...
	public static function &getInstance($id) {
		...
		if (!empty($gedrec)) {
			...
			if (empty($url) && empty($gedfile))
				return null;
			$server = ( !empty( $url ) && ( strtolower( $url ) != strtolower( $SERVER_URL ) ) )
				? new ServiceClient( $gedrec )
				: new LocalClient( $gedrec );
