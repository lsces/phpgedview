...
class MenuBar
{
	...
	* @return Menu the menu item
	*/
	public static function getMygedviewMenu() {
				...
				if ($rootid) {
					...
					foreach (array_unique($pids) as $pid) {
						if ($pid && $pid!=$rootid) {
							...
							$name = $person ? $person->getFullName() : $pid;
							$submenu = new Menu(
...
//			$menu->print_menu = null;
		...
		foreach ($reports as $file=>$report) {
			$label = !empty($report["title"][$LANGUAGE]) ? $report["title"][$LANGUAGE] : implode("", $report["title"] ?? [] );
		...
		foreach($menuList as $file=>$label) {
			...
			if ($report["access"]>=PGV_USER_ACCESS_LEVEL) {
				$label = $report["title"][$LANGUAGE] ?? implode("", $report["title"] ?? []);
				// indi report
				if ($pid) {
				...
				if (!empty($PGV_IMAGES["reports"]["small"]) && isset($PGV_IMAGES[$report["icon"]]["small"])) $iconfile=$PGV_IMAGE_DIR."/".$PGV_IMAGES[$report["icon"]]["small"];
	...
	* @return Menu|null the menu item
	*/
	public static function getClippingsMenu() {
...
//			$menu->print_menu = null;
	...
	* @return Menu the menu item
	*/
	public static function getPreviewMenu() {
			...
			while (false !== ($entry = $d->read())) {
				if ($entry[0]!='.' && $entry!='CVS' && is_dir(PGV_ROOT.'modules/'.$entry)) {
					if (file_exists(PGV_ROOT.'modules/'.$entry.'/menu.php')) {
						...
						$menu_class = '\\Bitweaver\\Phpgedview\\'.$entry.'_ModuleMenu';
						$obj=new $menu_class();
...
//			$menu->print_menu = null;
		...
		$menu->addSeparator();
		$submenu = $_SESSION["show_context_help"]
			? new Menu( $pgv_lang["hide_context_help"], PGV_SCRIPT_NAME . normalize_query_string( $QUERY_STRING . "&amp;show_context_help=no" ) )
			: new Menu( $pgv_lang["show_context_help"], PGV_SCRIPT_NAME . normalize_query_string( $QUERY_STRING . "&amp;show_context_help=yes" ) );
		$submenu->addClass("submenuitem$ff", "submenuitem_hover$ff", "", "icon_small_menu_help");
		$menu->addSubmenu($submenu);
		return $menu;
	}
	...
	*/
	public static function getLanguageMenu() {
		...
		$current = PGV_USER_ID ? $LANGUAGE : get_user_setting( PGV_USER_ID, 'language' );
...
//			$menu->print_menu = null;
	...
	* @return Menu the menu item
	*/
	public static function getFavouritesMenu() {		// Don't break custom themes using the old name
