...
class TreeNav {
	public $rootPerson = null;
	public $bwidth = 170;
	public $zoomLevel = 0;
	public $name = 'nav';
	public $generations = 4;
	public $allSpouses = true;
	public $images = true;
	...
	*/
	public function __construct($rootid='', $name='nav', $zoom=0) {
		...
		if (!empty($_REQUEST['navAjax'])) {
			...
			if ($_REQUEST['navAjax']=='embed') {
				...
				document.writeln('<script src="<?php print $SERVER_URL; ?>/js/phpgedview.js"></script>');
				<?php
				ob_start();
			...
			if (isset($_REQUEST['allSpouses'])) {
				$this->allSpouses = $_REQUEST['allSpouses']=='false' || $_REQUEST['allSpouses']==false ? false : true;
			}
	...
	*/
	public function drawViewport($id='', $width='', $height='') {
	...
	*/
	public function setupJS() {
		...
		?>
	<script src="<?php print $SERVER_URL; ?>js/treenav.js"></script>
	<script>
	<!--
		var myrules = {
	...
	*/
	public function drawChildren(&$family, $gen=2) {
	...
	* @param int $gen The number of generations of descendents to draw
	*/
	public function drawAllChildren(&$person, $gen=2) {
	...
	* @return string
	*/
	public function getThumbnail(&$person) {
	...
	*/
	public function drawPersonAllSpouses(&$person, $gen, $state) {
		global $SHOW_ID_NUMBERS, $PGV_IMAGE_DIR, $PGV_IMAGES, $TEXT_DIRECTION, $SERVER_URL;

		if ($gen<0) {
			return;
		}
		$style = $this->zoomLevel < -2 ? "display: none;" : "width: ".(10+$this->zoomLevel)."; height: ".(10+$this->zoomLevel).";";
		if (empty($person)) $person = $this->rootPerson;
	...
	*/
	public function drawPerson(&$person, $gen, $state, &$pfamily) {
		global $SHOW_ID_NUMBERS, $PGV_IMAGE_DIR, $PGV_IMAGES, $TEXT_DIRECTION, $SERVER_URL;
