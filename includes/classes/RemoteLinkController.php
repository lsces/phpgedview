...
use Bitweaver\Phpgedview\FamilySearchWrapper;
...
class RemoteLinkController extends BaseController {
	public $has_familysearch=null;
	public $pid=null;
	public $person=null;
	public $server_list=null;
	public $gedcom_list=null;
	...
	public function init() {
		...
		if (is_null($this->form_location)) {
			$this->form_location = $this->server_list ? 'existing' : 'remote';
		}
	...
	public function addRemoteServer($title, $url, $gedcom_id, $username, $password) {
	...
	// @param string $password
	// @return mixed the serverID of the server to link to
	public function addFamilySearchServer($title, $url, $gedcom_id, $username, $password) {
		$serverID = $this->checkExistingServer($url, $gedcom_id);
		if (!$serverID) {
			$gedcom_string = "0 @new@ SOUR\n";
...
//					$title = $service->getServiceTitle();
//				}
				$title = 'TODO'; // $service->getServiceTitle();
	...
	// @param string $gedcom_id
	// @return mixed the serverID of the server to link to
	public function addLocalServer($title, $gedcom_id) {
		global $SERVER_URL;
	...
	public function checkExistingServer($url, $gedcom_id='') {
		global $pgv_changes;
	...
	public function addLink() {
		...
		if ($serverID && $link_pid) {
			$indirec = isset( $pgv_changes["{$this->pid}_$GEDCOM"] ) ? find_updated_record( $this->pid, PGV_GED_ID ) : find_person_record( $this->pid, PGV_GED_ID );
			...
			case "daughter":
				...
				$gedcom_fam = ( get_gedcom_value( 'SEX', 1, $indirec, 0, false ) == 'F' )
					? "0 @new@ FAM\n1 WIFE @{$this->pid}@\n1 CHIL @{$stub_id}@"
					: "0 @new@ FAM\n1 HUSB @{$this->pid}@\n1 CHIL @{$stub_id}@";
				$fam_id=append_gedrec($gedcom_fam);
				$indirec.="\n1 FAMS @{$fam_id}@";
