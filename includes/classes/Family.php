...
class Family extends GedcomRecord {
	...
	public function __construct($data, $simple=true) {
		if (is_array($data)) {
			...
			$this->numChildren = preg_match('/^1 NCHI (\d+)/m', $data, $match)
				? $match[1] : count($this->childrenIds);
			// Check for divorce, etc. *before* we privatize the data so
			// we can correctly label spouses/ex-spouses/partners
			$this->_isDivorced=(bool)preg_match('/\n1 ('.PGV_EVENTS_DIV.')( Y|\n)/', $data);
			$this->_isNotMarried=(bool)preg_match('/\n1 _NMR( Y|\n)/', $data);
		...
		if ($this->husb && $this->husb->getSex()=='F' || $this->wife && $this->wife->getSex()=='M') {
			[ $this->husb, $this->wife ] = [ $this->wife, $this->husb ];
		}
	...
	public function getSpouse( &$person ) {
		if ( $this->wife === null or $this->husb === null ) return null;
	 ...
	 * @return string
	 */
	public function getSpouseId($pid) {
		if ( $this->wife === null or $this->husb === null ) return null;
	...
	public static function CompareMarrDate($x, $y) {
	...
	public function getNumberOfChildren() {
		...
		$this->numChildren = (int)get_gedcom_value('NCHI', 1, $this->gedrec);
	 ...
	 */
	public function getUpdatedFamily() {
	 ...
	 * @param Person $person
	 */
	public function hasParent(&$person) {
	 ...
	 * @param Person $person
	 */
	public function hasChild(&$person) {
		if (is_null($person)) return false;
		$this->loadChildren();
	 ...
	 */
	public function _parseMarriageRecord() {
	 ...
	 * @return string
	 */
	public function getMarriageRecord() {
		if (is_null($this->marriage)) $this->_parseMarriageRecord();
	...
	public function isDivorced() {
		return $this->_isDivorced;
	}
	public function isNotMarried() {
	 ...
	 * @return string
	 */
	public function getMarriageYear($est = true, $cal = ''){
	 ...
	 * @return string
	 */
	public function getMarriageMonth($est = true, $cal = ''){
		// TODO - change the design to use julian days, not gregorian years.
		$mdate = $this->getMarriageDate();
		$mdate=$mdate->MinDate();
		if ($cal) $mdate = $mdate->convert_to_cal($cal);
