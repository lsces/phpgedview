...
class AdvancedSearchController extends SearchController {
	 ...
	 * constructor
	 */
	public function AdvancedSearchController() {
		parent::__construct();
	}
	/**
	...
	public function getLabel($tag) {
		...
		foreach ( [ 'SDX', 'SDX_DM', 'SDX_STD', 'EXACT', 'BEGINS', 'CONTAINS' ] as $keyword) {
	...
	public function advancedSearch($justSql=false, $table="individuals", $prefix="i") {
		global $TBLPREFIX, $gBitDb, $gedcom_record_cache;
		...
		$sql = '';
		$sqlfields = $justSql ? "SELECT DISTINCT {$prefix}_id, {$prefix}_file" : "SELECT i_id, i_gedcom, i_isdead, i_file, i_sex";
		...
		for($i=0; $i<$fct; $i++) {
			...
			if ($parts[0]=="NAME") {
				...
				case 'SDX_STD':
					...
					foreach ($sdx as $k=>$v) {
						$sdx[$k] = $parts[1] == 'GIVN' ? "n_soundex_givn_std LIKE '%{$v}%'" : "n_soundex_surn_std LIKE '%{$v}%'";
					}
				...
				case 'SDX_DM':
					...
					foreach ($sdx as $k=>$v) {
						$sdx[$k] = $parts[1] == 'GIVN' ? "n_soundex_givn_dm LIKE '%{$v}%'" : "n_soundex_surn_dm LIKE '%{$v}%'";
					}
					$sqlwhere.=' AND ('.implode(' OR ', $sdx).')';
					break;
				case 'EXACT':
					// Exact match.
						$sqlwhere = match ( $parts[1] ) {
							'GIVN'  => " AND (n_givn  LIKE '{$value}' OR n_givn LIKE '{$value}%' OR n_givn LIKE.'%{$value}.OR n_givn LIKE.'%{$value}%'",
							'SURN'  => " AND n_surname  LIKE '{$value}'",
							default => " AND n_full  LIKE '{$value}'",
						};
					break;
				case 'BEGINS':
					// "Begins with" match.
						$sqlwhere = match ( $parts[1] ) {
							'GIVN'  => " AND (n_givn  LIKE '{$value}%' OR n_givn LIKE '% {$value}%'",
							'SURN'  => " AND n_surname  LIKE '{$value}%",
							default => " AND n_full  LIKE .'{$value}%'",
						};
					break;
			...
			//-- handle dates
				...
				if ($date->isOK()) {
					...
					$jd2 = $date->date2 ? $date->date2->maxJD : $date->date1->maxJD;
			...
			//-- handle parent/spouse names
				...
				for($j=0; $j<$fct; $j++) {
					...
					$this->fields[$j] = ( preg_match( "/^$parts[0]:/", $this->fields[$j] ) == 0 ) ? '' : preg_replace( "/^$parts[0]:/", "", $this->fields[$j] );
				}
...
//		print $sql;
	...
	public function PrintResults() {
		...
		if (count($this->myindilist)>0) {
			...
			uasort($this->myindilist, [ '\Bitweaver\Phpgedview\GedcomRecord', 'Compare' ]);
