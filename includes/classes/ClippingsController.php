		...
		if ($this->action == 'download') {
			...
			if ($this->filetype == "gedcom") {
				...
				if ($convert == "yes") {
					...
					$filetext = mb_convert_encoding($filetext, "UTF-8", mb_detect_encoding($filetext));
				}
				...
				for ($i = 0; $i < $ct; $i++) {
					...
					if ($clipping['gedcom'] == $GEDCOM) {
						...
						if ($convert == "yes")
						$record = mb_convert_encoding($record, "UTF-8", mb_detect_encoding($record));
...
//								$record = preg_replace("|(\d FILE )" . addslashes($match[$k][1]) . "|", "$1" . $filename, $record);
			...
			if ($this->filetype == "gramps") {
				...
				if ($convert)
					$this->download_data = mb_convert_encoding($this->download_data, "UTF-8", mb_detect_encoding($this->download_data));
	...
	function zip_cart()
	{
		...
		$fp = fopen(PHPGEDVIEW_PKG_INDEX_PATH .$tempFileName, "wb");
		if($fp)
		{
			flock($fp,LOCK_EX);
			...
			$fname = PHPGEDVIEW_PKG_INDEX_PATH .$zipName;
			...
			$this->media_list[]= array (PCLZIP_ATT_FILE_NAME => PHPGEDVIEW_PKG_INDEX_PATH .$tempFileName, PCLZIP_ATT_FILE_NEW_FULL_NAME => $tempFileName);
			...
			unlink(PHPGEDVIEW_PKG_INDEX_PATH .$tempFileName);
		}
		else
		{
			print $pgv_lang["um_file_create_fail2"]." ".PHPGEDVIEW_PKG_INDEX_PATH ."$tempFileName ".$pgv_lang["um_file_create_fail3"]."<br /><br />";
		}
	}
	/**
