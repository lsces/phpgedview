...
class AncestryControllerRoot extends BaseController {
	...
	function init() {
		...
		$this->cellwidth = strlen($this->name)<30 ? "420" : strlen($this->name) * 14;
	...
	function print_child_ascendancy($pid, $sosa, $depth) {
		...
		print "</td>";
		print "<td>";
		$label = $TEXT_DIRECTION=="ltr"
			? $pgv_lang["ancestry_chart"].": ".$pid
			: $pid." :".$pgv_lang["ancestry_chart"];
		...
		$relation ="";
		if (!$new && !empty($pidarr["$pid"])) $relation = "<br />[=<a href=\"#sosa".$pidarr["$pid"]."\">".$pidarr["$pid"]."</a> - ".get_sosa_name($pidarr["$pid"])."]";
