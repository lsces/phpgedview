...
use Bitweaver\Phpgedview\Person\getBirthDate;
use Bitweaver\Phpgedview\Person\isDead;
use Bitweaver\Phpgedview\Person\getSex;
...
function print_pedigree_person($pid, $style=1, $show_famlink=true, $count=0, $personcount="1") {
	...
	if ($person->canDisplayName()) {
		if ($show_famlink && (empty($SEARCH_SPIDER))) {
			if ($LINK_ICONS!="disabled") {
				...
				// NOTE: Zoom
				$title = ( $TEXT_DIRECTION == "ltr" ) ? $pgv_lang["pedigree_chart"] . ": " . $pid : $pid . " :" . $pgv_lang["pedigree_chart"];
				...
				if (file_exists( PGV_ROOT . 'modules/googlemap/pedigree_map.php' )) {
					...
					$title = ( $TEXT_DIRECTION == "ltr" ) ? $pgv_lang["pedigree_map"] . ": " . $pid : $pid . " :" . $pgv_lang["pedigree_map"];
				...
				if (!empty( $username )) {
					...
					if ($myid && $myid != $pid) {
						$title = ( $TEXT_DIRECTION == "ltr" ) ? $pgv_lang["relationship_chart"] . ": " . $pid : $pid . " :" . $pgv_lang["relationship_chart"];
				...
				$title = ( $TEXT_DIRECTION == "ltr" ) ? $pgv_lang["descend_chart"] . ": " . $pid : $pid . " :" . $pgv_lang["descend_chart"];
				...
				if (file_exists( PGV_ROOT . 'ancestry.php' )) {
					$title = ( $TEXT_DIRECTION == "ltr" ) ? $pgv_lang["ancestry_chart"] . ": " . $pid : $pid . " :" . $pgv_lang["ancestry_chart"];
				...
				if (file_exists( PGV_ROOT . 'compact.php' )) {
					$title = ( $TEXT_DIRECTION == "ltr" ) ? $pgv_lang["compact_chart"] . ": " . $pid : $pid . " :" . $pgv_lang["compact_chart"];
				...
				if (file_exists( PGV_ROOT . 'fanchart.php' ) and defined( "IMG_ARC_PIE" ) and function_exists( "imagettftext" )) {
					$title = ( $TEXT_DIRECTION == "ltr" ) ? $pgv_lang["fan_chart"] . ": " . $pid : $pid . " :" . $pgv_lang["fan_chart"];
				...
				if (file_exists( PGV_ROOT . 'hourglass.php' )) {
					$title = ( $TEXT_DIRECTION == "ltr" ) ? $pgv_lang["hourglass_chart"] . ": " . $pid : $pid . " :" . $pgv_lang["hourglass_chart"];
				...
				if (file_exists( PGV_ROOT . 'treenav.php' )) {
					$title = ( $TEXT_DIRECTION == "ltr" ) ? $pgv_lang["interactive_tree"] . ": " . $pid : $pid . " :" . $pgv_lang["interactive_tree"];
				...
				foreach ( $fams as $famid => $family ) {
					if (!is_null( $family )) {
						...
						if (( !empty( $spouse ) ) || ( $num > 0 )) {
							$title = ( $TEXT_DIRECTION == "ltr" ) ? $pgv_lang["familybook_chart"] . ": " . $famid : $famid . " :" . $pgv_lang["familybook_chart"];
							...
							if (!empty( $spouse )) {
								$title = ( $TEXT_DIRECTION == "ltr" ) ? $pgv_lang["indi_info"] . ": " . $spouse->getXref() : $spouse->getXref() . " :" . $pgv_lang["indi_info"];
								$personlinks .= "<a href=\"" . encode_url( $spouse->getLinkUrl() ) . "\" title=\"$title\" $mouseAction1>";
								if ($spouse->canDisplayName())
									$personlinks .= PrintReady( $spouse->getFullName() );
								else
									$personlinks .= $pgv_lang["private"];
								$personlinks .= "</a><br />";
							}
						}
						/* @var $child Person */
						foreach ( $children as $c => $child ) {
							$title = ( $TEXT_DIRECTION == "ltr" ) ? $pgv_lang["indi_info"] . ": " . $child->getXref() : $child->getXref() . " :" . $pgv_lang["indi_info"];
							$personlinks .= "&nbsp;&nbsp;<a href=\"" . encode_url( $child->getLinkUrl() ) . "\" title=\"$title\" $mouseAction1>";
							if ($child->canDisplayName())
								$personlinks .= PrintReady( $child->getFullName() );
							else
								$personlinks .= $pgv_lang["private"];
							$personlinks .= "<br /></a>";
						}
			...
			if ($style == 1)
				$outBoxAdd .= " class=\"person_box$isF\" style=\"width: " . $bwidth . "px; height: " . $bheight . "px; padding: 2px; overflow: hidden; z-index:'-1';\"";
			else
				$outBoxAdd .= " style=\"padding: 2px;\"";
			// NOTE: Zoom
			if (( $ZOOM_BOXES != "disabled" ) && ( !$show_full )) {
				if ($ZOOM_BOXES == "mouseover")
					$outBoxAdd .= $mouseAction2;
				if ($ZOOM_BOXES == "mousedown")
					$outBoxAdd .= $mouseAction3;
			...
			if ($show_full)
				$iconsStyleAdd .= " display: block;";
			else
				$iconsStyleAdd .= " display: none;";
			//echo "\">";
			// NOTE: Zoom
			if (( $ZOOM_BOXES != "disabled" ) && $show_full) {
			...
			if ($LINK_ICONS != "disabled") {
				...
				$title = empty( $whichChart ) ? "" : ( $TEXT_DIRECTION == "ltr" ? $pgv_lang[$whichChart] . ": " . $whichID : $whichID . " :" . $pgv_lang[$whichChart]) ;
				$icons .= "<a href=\"$click_link\" title=\"$title\" ";
				// NOTE: Zoom
				if ($LINK_ICONS == "mouseover")
					$icons .= "onmouseover=\"show_family_box('" . $boxID . "', '";
				if ($LINK_ICONS == "click")
					$icons .= "onclick=\"toggle_family_box('" . $boxID . "', '";
				if ($style == 1)
					$icons .= "box$pid";
				else
					$icons .= "relatives";
	...
	$title = ( $TEXT_DIRECTION == "ltr" ) ? $pgv_lang["indi_info"] . ": " . $pid : $pid . " :" . $pgv_lang["indi_info"];
	...
	foreach ( $cssfacts as $indexval => $fact ) {
		if (strpos( $indirec, "1 $fact" ) !== false)
			$classfacts .= " $fact";
	}
	...
	if ($SHOW_ID_NUMBERS) {
		if ($TEXT_DIRECTION == "ltr")
			$showid .= "<span class=\"details$style\">" . getLRM() . "($pid)" . getLRM() . " </span>";
		else
			$showid .= "<span class=\"details$style\">" . getRLM() . "($pid)" . getRLM() . " </span>";
	}
	if (strlen( $addname ?? '' ) > 0) {
		$tempStyle = $style;
...
function print_header( $title, $head = "", $use_alternate_styles = true )
{
	...
	global $view, $cart;
	global $CHARACTER_SET, $PGV_IMAGE_DIR, $GEDCOM, $GEDCOM_TITLE, $CONTACT_EMAIL, $COMMON_NAMES_THRESHOLD;
	...
	$metaTitle = ( empty( $META_TITLE ) ) ? ' - ' . PGV_PHPGEDVIEW : " - " . $META_TITLE . ' - ' . PGV_PHPGEDVIEW;

	$title = PrintReady( stripLRMRLM( strip_tags( $title . $metaTitle ) ) );
	...
	$javascript = '';
	$query_string = isset( $changelanguage ) ? normalize_query_string( $QUERY_STRING . "&amp;changelanguage=&amp;NEWLANGUAGE=" ) : $QUERY_STRING;
	if ($view != "preview") {
		...
		if (empty( $META_AUTHOR ) || empty( $META_PUBLISHER ) || empty( $META_COPYRIGHT )) {
			...
			if ($user_id) {
				...
				if (empty( $META_AUTHOR ))
					$META_AUTHOR = $cuserName;
				if (empty( $META_PUBLISHER ))
					$META_PUBLISHER = $cuserName;
				if (empty( $META_COPYRIGHT ))
					$META_COPYRIGHT = $cuserName;
			}
		}
	...
	//-->
	' . PGV_JS_END . '<script src="js/phpgedview.js" language="JavaScript""></script>';
	$bodyOnLoad = '';
	if ($view == "preview")
		$bodyOnLoad .= " onbeforeprint=\"hidePrint();\" onafterprint=\"showBack();\"";
	$bodyOnLoad .= " onload=\"";
	if (!empty( $ONLOADFUNCTION ))
		$bodyOnLoad .= $ONLOADFUNCTION;
	if ($TEXT_DIRECTION == "rtl") {
...
function linkWebAnalytics()
{
	 ...
	 *              New code format. <script id="clustrmaps" src="//cdn.clustrmaps.com/map_v2.js?u=XXXX&d=YYYY"></script>
	...
	if (defined( 'PGV_CLUSTRMAPS_SITE' ) && defined( 'PGV_CLUSTRMAPS_SERVER' )) {
		$result .= '<!-- ClustrMaps -->' . "\n";
		$result .= '<script id="clustrmaps" src="//cdn.clustrmaps.com/map_v2.js?u=' . PGV_CLUSTRMAPS_SITE . '&d=' . PGV_CLUSTRMAPS_SERVER . '"></script>' . "\n";
...
{
	global $start_time, $pgv_lang, $PRIVACY_CHECKS, $gBitDb;
...
{ // This function is used by 3rd party themes.
	echo contact_links();
}
...
function contact_links()
{
	...
	$contact_user_id = ( $WEBMASTER_EMAIL == $CONTACT_EMAIL ) ? $webmaster_user_id : get_user_id( $CONTACT_EMAIL );

	$supportLink = user_contact_link( $webmaster_user_id, $SUPPORT_METHOD );
	$contactLink = ( $webmaster_user_id == $contact_user_id && $SUPPORT_METHOD == $CONTACT_METHOD ) ? $supportLink : user_contact_link( $contact_user_id, $CONTACT_METHOD );
...
function contact_menus()
{
	...
	$contact_menu = ( $webmaster_user_id == $contact_user_id && $SUPPORT_METHOD == $CONTACT_METHOD ) ? $support_menu : user_contact_menu( $contact_user_id, $CONTACT_METHOD );
...
{
	global $pgv_lang, $GEDCOM, $SHOW_ID_NUMBERS, $QUERY_STRING;
	...
	if (!PGV_USER_NAME && $REQUIRE_AUTHENTICATION)
		return false;
	...
	if (!PGV_USER_NAME && count( $gedcomfavs ) == 0)
		return;
...
function print_note_record( $text, $nlevel, $nrec, $textOnly = false, $return = false )
{
	...
	if (!isset( $EXPAND_NOTES ))
		$EXPAND_NOTES = $EXPAND_SOURCES; // FIXME
	$elementID = "N-" . floor( microtime( 1 ) * 1000000 );
	$text = trim( $text );
	...
	if (!empty( $text ) || !empty( $centitl )) {
		...
		if ($brpos !== false) {
			...
			if ($plusminus == "plus")
				$data .= $pgv_lang["show_details"] . "\" title=\"" . $pgv_lang["show_details"] . "\" /></a> ";
			else
				$data .= $pgv_lang["hide_details"] . "\" title=\"" . $pgv_lang["hide_details"] . "\" /></a> ";
		}
...
function print_help_index( $help )
{
	...
	uasort( $help_sorted, "\Bitweaver\Phpgedview\stringsort" );
	if ($ch == 0)
		$ch = count( $help_sorted );
	else
		$ch += $ch;
	if ($ch > 0)
		echo "<table width=\"100%\"><tr><td style=\"vertical-align: top;\"><ul>";
	$i = 0;
	foreach ( $help_sorted as $k => $help_item ) {
		echo "<li>", $k, "</li>";
		$i++;
		if ($i == ceil( $ch / 2 ))
			echo "</ul></td><td style=\"vertical-align: top;\"><ul>";
	}
	if ($ch > 0)
		echo "</ul></td></tr></table>";
}
 ...
 * @return string formated for printing
 */
function PrintReady( $text, $InHeaders = false, $trim = true )
{
	...
	$text = ( !$InHeaders ) ? str_replace( array( "\x02\x01", "\x02 \x01", "\x01", "\x02" ), array( "", " ", "<span class=\"search_hit\">", "</span>" ), $text ) : str_replace( array( "\x02\x01", "\x02 \x01", "\x01", "\x02" ), array( "", " ", "", "" ), $text );
	return $text;
}
	...
	for ( $i = 0; $i < $ct; $i++ ) {
		...
		if (strstr( $gedrec, "@ INDI" ) !== false || strstr( $gedrec, "@ SUBM" ) !== false) {
			...
			if (preg_match( "/2 DATE (.*)/", $factrec, $dmatch )) {
				...
				$death_date = $death_event ? $death_event->getDate() : new GedcomDate( '' );
				$ageText = '';
				...
				if (!strstr( $factrec, "_BIRT_" ) && !strstr( $factrec, "_DEAT_" ) && GedcomDate::Compare( $event_date, $death_date ) >= 0 && $tmp->isDead()) {
					...
					if (!empty( $age )) {
						$ageText = ( GedcomDate::GetAgeGedcom( $death_date, $event_date ) == "0d" ) ? "(" . $pgv_lang["at_death_day"] . ")" : "(" . $age . " " . $pgv_lang["after_death"] . ")";
					}
				}
			...
			if ($view != "preview" && !$autoRela) {
				if ($type == 'FAM') {
					...
					if ($famrec) {
						...
						$pid1 = $parents["HUSB"];
						if ($pid1 && $pid1 != $pid2)
							echo " - <a href=\"", encode_url( "relationship.php?show_full={$PEDIGREE_FULL_DETAILS}&pid1={$pid1}&pid2={$pid2}&pretty=2&followspouse=1&ged=" . PGV_GED_ID ), "\">[", $pgv_lang["relationship_chart"], "<img src=\"$PGV_IMAGE_DIR/", $PGV_IMAGES["sex"]["small"], "\" title=\"", $pgv_lang["husband"], "\" alt=\"", $pgv_lang["husband"], "\" class=\"gender_image\" />]</a>";
						$pid1 = $parents["WIFE"];
						if ($pid1 && $pid1 != $pid2)
							echo " - <a href=\"", encode_url( "relationship.php?show_full={$PEDIGREE_FULL_DETAILS}&pid1={$pid1}&pid2={$pid2}&pretty=2&followspouse=1&ged=" . PGV_GED_ID ), "\">[", $pgv_lang["relationship_chart"], "<img src=\"$PGV_IMAGE_DIR/", $PGV_IMAGES["sexf"]["small"], "\" title=\"", $pgv_lang["wife"], "\" alt=\"", $pgv_lang["wife"], "\" class=\"gender_image\" />]</a>";
					}
				}
				else if ($pid != $pid2 && $rela != "twin_sister")
					echo " - <a href=\"", encode_url( "relationship.php?show_full={$PEDIGREE_FULL_DETAILS}&pid1={$pid}&pid2={$pid2}&pretty=2&followspouse=1&ged=" . PGV_GED_ID ), "\">[", $pgv_lang["relationship_chart"], "]</a>";
			}
...
function format_fact_date( &$eventObj, $anchor = false, $time = false )
{
	...
	if (!is_object( $eventObj ))
		pgv_error_handler( 2, "Must use Event object", __FILE__, __LINE__ );
		...
		if (!is_null( $person ) && $person->getType() == 'INDI') {
			...
			// age at event
				...
				if ($fact != 'DEAT' && GedcomDate::Compare( $date, $death_date ) >= 0) {
					...
					if ($age != '') {
						$ageText = ( GedcomDate::GetAgeGedcom( $death_date, $date ) == "0d" ) ? '(' . $pgv_lang['at_death_day'] . ')' : '(' . $age . ' ' . $pgv_lang['after_death'] . ')';
					}
				}
				if ($ageText != '')
					$html .= '<span class="age"> ' . PrintReady( $ageText ) . '</span>';
			}
		}
...
function print_add_new_fact( $id, $usedfacts, $type )
{
	...
	if (!empty( $_SESSION["clipboard"] )) {
		...
		foreach ( array_reverse( $_SESSION["clipboard"], true ) as $key => $fact ) {
			if ($fact["type"] == $type || $fact["type"] == 'all') {
				...
				$fact_type = ( array_key_exists( $fact['fact'], $factarray ) ) ? $factarray[$fact['fact']] : $fact['fact'];
	...
	if (( $type == "INDI" ) || ( $type == "FAM" ))
		echo "<option value=\"EVEN\">", $pgv_lang["custom_event"], " [EVEN]</option>";
	echo "\n</select>\n";
...
function print_findindi_link( $element_id, $indiname, $asString = false, $multiple = false, $ged = '', $filter = '' )
{
	...
	if (empty( $ged ))
		$ged = $GEDCOM;
	$Link = isset( $PGV_IMAGES["indi"]["button"] ) ? "<img src=\"" . $PGV_IMAGE_DIR . "/" . $PGV_IMAGES["indi"]["button"] . "\" alt=\"" . $text . "\" title=\"" . $text . "\" border=\"0\" align=\"middle\" />" : $text;
...
function print_findplace_link( $element_id, $ged = '', $asString = false )
{
	...
	$Link = isset( $PGV_IMAGES["place"]["button"] ) ? "<img src=\"" . $PGV_IMAGE_DIR . "/" . $PGV_IMAGES["place"]["button"] . "\" alt=\"" . $text . "\" title=\"" . $text . "\" border=\"0\" align=\"middle\" />" : $text;
...
function print_findfamily_link( $element_id, $ged = '', $asString = false )
{
	...
	$Link = isset( $PGV_IMAGES["family"]["button"] ) ? "<img src=\"" . $PGV_IMAGE_DIR . "/" . $PGV_IMAGES["family"]["button"] . "\" alt=\"" . $text . "\" title=\"" . $text . "\" border=\"0\" align=\"middle\" />" : $text;
...
function print_specialchar_link( $element_id, $vert, $asString = false )
{
	...
	$Link = isset( $PGV_IMAGES["keyboard"]["button"] ) ? "<img id=\"" . $element_id . "_spec\" name=\"" . $element_id . "_spec\" src=\"" . $PGV_IMAGE_DIR . "/" . $PGV_IMAGES["keyboard"]["button"] . "\"  alt=\"" . $text . "\"  title=\"" . $text . "\" border=\"0\" align=\"middle\" />" : $text;
...
function print_findsource_link( $element_id, $sourcename = "", $asString = false, $ged = '' )
{
	...
	$Link = isset( $PGV_IMAGES["source"]["button"] ) ? "<img src=\"" . $PGV_IMAGE_DIR . "/" . $PGV_IMAGES["source"]["button"] . "\" alt=\"" . $text . "\" title=\"" . $text . "\" border=\"0\" align=\"middle\" />" : $text;
...
function print_findnote_link( $element_id, $notename = "", $asString = false, $ged = '' )
{
	...
	$Link = isset( $PGV_IMAGES["note"]["button"] ) ? "<img src=\"" . $PGV_IMAGE_DIR . "/" . $PGV_IMAGES["note"]["button"] . "\" alt=\"" . $text . "\" title=\"" . $text . "\" border=\"0\" align=\"middle\" />" : $text;
...
function print_findrepository_link( $element_id, $ged = '', $asString = false )
{
	...
	$Link = isset( $PGV_IMAGES["repository"]["button"] ) ? "<img src=\"" . $PGV_IMAGE_DIR . "/" . $PGV_IMAGES["repository"]["button"] . "\" alt=\"" . $text . "\" title=\"" . $text . "\" border=\"0\" align=\"middle\" />" : $text;
...
function print_findmedia_link( $element_id, $choose = "", $ged = '', $asString = false )
{
	...
	$Link = isset( $PGV_IMAGES["media"]["button"] ) ? "<img src=\"" . $PGV_IMAGE_DIR . "/" . $PGV_IMAGES["media"]["button"] . "\" alt=\"" . $text . "\" title=\"" . $text . "\" border=\"0\" align=\"middle\" />" : $text;
