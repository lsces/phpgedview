...
function checkChangeTime($pid, $gedrec, $last_time) {
	...
	if (isset($pgv_changes[$pid."_".PGV_GEDCOM])) {
		...
		$cdate = get_gedcom_value("DATE", 2, $changrec, 0, false);
		if (!empty($cdate)) {
			$ctime = get_gedcom_value("DATE:TIME", 2, $changrec);
			$changeUser = get_gedcom_value("_PGVU", 2, $changrec, 0, false);
	...
	if (isset($_REQUEST['linenum']) && $changeTime!=0 && $last_time && $changeTime > $last_time) {
		...
		if (!empty($changeUser)) echo str_replace( [ "#CHANGEUSER#", "#CHANGEDATE#" ], [ $changeUser, date( "d M Y H:i:s", $changeTime ) ], $pgv_lang["edit_concurrency_change"]), "<br /><br />";
...
function replace_gedrec($gid, $gedrec, $chan=true, $linkpid='') {
	...
	if (($gedrec = check_gedcom($gedrec, $chan))!==false) {
		...
		if ($ct>0) {
			...
			if (!isset($pgv_changes[$gid."_".PGV_GEDCOM])) $pgv_changes[$gid."_".PGV_GEDCOM] = [];
			else {
...
function append_gedrec($gedrec, $chan=true, $linkpid='') {
	...
	if (($gedrec = check_gedcom($gedrec, $chan))!==false) {
		...
		$xref = preg_match("/\d+/", $gid)==0 ? get_new_xref($type) : $gid;
		...
		if (!empty($linkpid)) $change["linkpid"] = $linkpid;
		$change["undo"] = reformat_record_import($gedrec);
		if (!isset($pgv_changes[$xref."_".PGV_GEDCOM])) $pgv_changes[$xref."_".PGV_GEDCOM] = [];
...
function print_indi_form($nextaction, $famid, $linenum='', $namerec='', $famtag="CHIL", $sextag='') {
	...
	if (empty($namerec)) {
		...
		$famrec = isset($pgv_changes[$famid."_".PGV_GEDCOM])#
			? find_updated_record($famid, PGV_GED_ID)
			: find_family_record($famid, PGV_GED_ID);
		...
		$prec = isset($pgv_changes[$pid."_".PGV_GEDCOM])
			? find_updated_record($pid, PGV_GED_ID)
			: find_person_record($pid, PGV_GED_ID);
			...
			case 'addspouseaction':
				if ($famtag=='WIFE' && preg_match('/\/(.*)\//', $indi_name, $match)) {
					if ($SURNAME_TRADITION=='polish') {
						$match[1]=preg_replace( [ '/ski$/', '/cki$/', '/dzki$/' ], [ 'ska', 'cka', 'dzka' ], $match[1]);
					}
					$new_marnm=$match[1];
				}
				break;
			case 'addchildaction':
				if (preg_match('/\/((?:[a-z]{2,3}\s+)*)(.*)\//i', $father_name, $match)) {
					if ($SURNAME_TRADITION=='polish' && $sextag=='F') {
						$match[2]=preg_replace( [ '/ski$/', '/cki$/', '/dzki$/' ], [ 'ska', 'cka', 'dzka' ], $match[2]);
					}
			...
			case 'addnewparentaction':
				if ($famtag=='HUSB' && preg_match('/\/((?:[a-z]{2,3}\s+)*)(.*)\//i', $indi_name, $match)) {
					if ($SURNAME_TRADITION=='polish' && $sextag=='M') {
						$match[2]=preg_replace( [ '/ska$/', '/cka$/', '/dzka$/' ], [ 'ski', 'cki', 'dzki' ], $match[2]);
					}
					$name_fields['SPFX']=trim($match[1]);
					$name_fields['SURN']=$match[2];
					$name_fields['NAME']="/{$match[1]}{$match[2]}/";
...
function print_calendar_popup($id, $asString=false) {
	...
	$Link = isset($PGV_IMAGES["calendar"]["button"]) ? "<img src=\"".$PGV_IMAGE_DIR."/".$PGV_IMAGES["calendar"]["button"]."\" name=\"img".$id."\" id=\"img".$id."\" alt=\"".$text."\" title=\"".$text."\" border=\"0\" align=\"middle\" />" : $text;
	$out = ' ';
...
function print_addnewmedia_link($element_id) {
	...
	$Link = isset($PGV_IMAGES["addmedia"]["button"])
		? "<img src=\"".$PGV_IMAGE_DIR."/".$PGV_IMAGES["addmedia"]["button"]."\" alt=\"".$text."\" title=\"".$text."\" border=\"0\" align=\"middle\" />" : $text;
...
function print_addnewrepository_link($element_id) {
	...
	$Link = isset($PGV_IMAGES["addrepository"]["button"]) ? "<img src=\"".$PGV_IMAGE_DIR."/".$PGV_IMAGES["addrepository"]["button"]."\" alt=\"".$text."\" title=\"".$text."\" border=\"0\" align=\"middle\" />" : $text;
...
function print_addnewnote_link($element_id) {
	...
	$Link = isset($PGV_IMAGES["addnote"]["button"])
		? "<img src=\"".$PGV_IMAGE_DIR."/".$PGV_IMAGES["addnote"]["button"]."\" alt=\"".$text."\" title=\"".$text."\" border=\"0\" align=\"middle\" />" : $text;
...
function print_editnote_link($note_id) {
	...
	$Link = isset($PGV_IMAGES["note"]["button"])
		? "<img src=\"".$PGV_IMAGE_DIR."/".$PGV_IMAGES["note"]["button"]."\" alt=\"".$text."\" title=\"".$text."\" border=\"0\" align=\"middle\" />" : $text;
...
function print_addnewsource_link($element_id) {
	...
	$Link = isset( $PGV_IMAGES["addsource"]["button"] ) ? "<img src=\"" . $PGV_IMAGE_DIR . "/" . $PGV_IMAGES["addsource"]["button"] . "\" alt=\"" . $text . "\" title=\"" . $text . "\" border=\"0\" align=\"middle\" />" : $text;
...
function add_simple_tag( $tag, $upperlevel = '', $label = '', $readOnly = '', $noClose = '', $rowDisplay = true )
{
	...
	$element_name = ( $level == 0 ) ? ( $upperlevel ? $upperlevel . "_" . $fact : $fact ) : "text[]";
	if ($level == 1)
		$main_fact = $fact;

	// element id : used by javascript functions
	$element_id = ( $level == 0 ) ? $fact : $fact . floor( (float) microtime() * 1000000 ); // ex: SOUR56402
	if ($upperlevel)
		$element_id = "{$upperlevel}_$fact"; // ex: BIRT_DATE | DEAT_DATE ...

	// field value
	$islink = substr( $value ?? '', 0, 1 ) == "@" and substr( $value ?? '', 0, 2 ) != "@#";
	$value = $islink ? trim( trim( substr( $tag, strlen( $fact ) + 3 ) ), " @\r" ) : trim( substr( $tag, strlen( $fact ) + 3 ) );
	...
	if (in_array( $fact, $emptyfacts ) && ( empty( $value ) || $value == "y" || $value == "Y" )) {
		...
		*/
		...
		echo "<select tabindex=\"", $tabkey, "\" name=\"", $element_name, "\" >";
		foreach ( [
			"BOTH" => $factarray["HUSB"] . "+" . $factarray["WIFE"],
			"HUSB" => $factarray["HUSB"],
			"WIFE" => $factarray["WIFE"],
		] as $k => $v ) {
			echo "<option value='$k'";
		...
		foreach ( [ "none", "locked", "privacy", "confidential" ] as $resn_index => $resn_val ) {
			$resnv = ( $resn_val == "none" ) ? "" : $resn_val;
		...
		else if (( $cols > 20 || $fact == "NPFX" ) && $readOnly == '')
			print_specialchar_link( $element_id, false );
	}
	// MARRiage TYPE : hide text field and show a selection list
	if ($fact == "TYPE" and !empty( $tags[0] ) and $tags[0] == "MARR") {
		echo "<script>";
		echo "document.getElementById('", $element_id, "').style.display='none'";
		echo "</script>";
		echo "<select tabindex=\"", $tabkey, "\" id=\"", $element_id, "_sel\" onchange=\"document.getElementById('", $element_id, "').value=this.value;\" >\n";
		foreach ( [ "Unknown", "Civil", "Religious", "Partners" ] as $indexval => $key ) {
			if ($key == "Unknown")
				echo "<option value=\"\"";
			else
				echo "<option value=\"", $key, "\"";
	...
	if ($readOnly == '') {
		...
		if ($fact == "SOUR") {
			...
			//print_autopaste_link($element_id, [ "S1", "S2" ], false, false, true);
...
function addNewName()
{
	...
	$tags = [ 'TYPE', 'NPFX', 'GIVN', 'SPFX', 'SURN', 'NSFX', '_MARNM' ];
...
function addNewFact( $fact )
{
	...
	if ($DATE || $PLAC || $FACT && $FACT != 'Y') {
		$gedrec = ( $FACT && $FACT != 'Y' ) ? "1 {$fact} {$FACT}\n" : "1 {$fact}\n";
		if ($DATE) {
...
function handle_updates( $newged, $levelOverride = "no" )
{
	...
	$levelAdjust = ( $levelOverride == "no" || count( $glevels ) == 0 ) ? 0 : $levelOverride - $glevels[0];
...
function check_input_date( $datestr )
{
	...
	$datestr = ( function_exists( $conversion_function ) ) ? $conversion_function( $datestr ) : default_edit_to_gedcom_date( $datestr );
...
function linkMedia( $mediaid, $linktoid, $level = 1, $chan = true )
{
	...
	if (empty( $level ))
		$level = 1;
	if ($level != 1)
		return false; // Level 2 items get linked elsewhere
	// find Indi, Family, or Source record to link to
	$gedrec = isset( $pgv_changes[$linktoid . "_" . PGV_GEDCOM] ) ? find_updated_record( $linktoid, PGV_GED_ID ) : find_gedcom_record( $linktoid, PGV_GED_ID );

	//-- check if we are re-editing an unaccepted link that is not already in the DB
	if (strpos( $gedrec, "1 OBJE @$mediaid@" ) !== false)
		return false;
...
function unlinkMedia( $linktoid, $linenum, $mediaid, $level = 1, $chan = true )
{
	...
	if (empty( $level ))
		$level = 1;
	if ($level != 1)
		return false; // Level 2 items get unlinked elsewhere (maybe ??)
	...
	while ( true ) {
		...
		if ($type != "DATA" && $type != "CONT") {
			...
			if ($type == 'DATE') {
				...
				$text = ( function_exists( $conversion_function ) ) ? $conversion_function( $text ) : default_gedcom_to_edit_date( $text );
			}
...
function delete_person( $pid, $gedrec = '' )
{
	...
	if (empty( $gedrec ))
		$gedrec = find_person_record( $pid, PGV_GED_ID );
	if (!empty( $gedrec )) {
		$success = true;
		...
		for ( $i = 0; $i < $ct; $i++ ) {
			...
			if (!empty( $famrec )) {
				...
				foreach ( $lines as $indexval => $line ) {
					...
					$level = ( $ct > 0 ) ? $levelmatch[1] : 1;
					//-- make sure we don't add any sublevel records
					if ($level <= $lastlevel)
						$lastlevel = -1;
					if (( strpos( $line, "@$pid@" ) === false ) && ( $lastlevel == -1 ))
						$newfamrec .= $line . "\n";
					else {
						$lastlevel = $level;
					}
				...
				if ($pt < 2) {
					for ( $j = 0; $j < $pt; $j++ ) {
						$xref = $pmatch[$j][1];
						if ($xref != $pid) {
							$indirec = ( !isset( $pgv_changes[$xref . "_" . PGV_GEDCOM] ) ) ? find_gedcom_record( $xref, PGV_GED_ID ) : find_updated_record( $xref, PGV_GED_ID );
...
function delete_family( $pid, $gedrec = '' )
{
	...
	global $pgv_lang;
	if (empty( $gedrec ))
		$gedrec = find_family_record( $pid, PGV_GED_ID );
	if (!empty( $gedrec )) {
		$success = true;
		...
		for ( $i = 0; $i < $ct; $i++ ) {
			...
			$indirec = ( !isset( $pgv_changes[$id . "_" . PGV_GEDCOM] ) ) ? find_gedcom_record( $id, PGV_GED_ID ) : find_updated_record( $id, PGV_GED_ID );
			if (!empty( $indirec )) {
				$lines = explode( "\n", $indirec );
				$newindirec = '';
				$lastlevel = -1;
