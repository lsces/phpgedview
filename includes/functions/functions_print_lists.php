...
function print_indi_table($datalist, $legend="", $option="") {
	...
	$table_id = "ID".intval(microtime(1)*1000000); // sorttable requires a unique ID
	...
	foreach($datalist as $key => $value) {
		...
		foreach ($names as $num=>$name) {
			...
			$title = ( $title = $name['type'] == '_MARNM' ) ? 'title="' . $factarray['_MARNM'] . '"' : '';
			if ($num == $primary) {
		...
		if ($tiny) {
			echo "<td class=\"list_value_wrap\">";
			if (showFactDetails( 'NCHI', $person->getXref() )) {
				if ($SEARCH_SPIDER) {
	...
	if ($hidden)
		echo "<br /><span class=\"warning\">", $pgv_lang["hidden"], " : ", $hidden, "</span>";
	echo "</td>";
...
function print_fam_table( $datalist, $legend = "", $option = "" )
{
	...
	echo "<td></td>";
	if ($SHOW_ID_NUMBERS)
		echo "<th class=\"list_label rela\">FAM</th>";
	if ($SHOW_ID_NUMBERS)
		echo "<th class=\"list_label rela\">INDI</th>";
	...
	foreach ( $datalist as $key => $value ) {
		if (is_object( $value )) { // Array of objects
			...
			if (isset( $value["gid"] ))
				$gid = $value["gid"];
			$family = isset( $value["gedcom"] ) ? new Family( $value["gedcom"] ) : Family::getInstance( $gid );
		}
		if (is_null( $family ))
			continue;
		if ($family->getType() !== "FAM")
			continue;
		...
		//-- place filtering
		if ($option == "MARR_PLAC" && strstr( $family->getMarriagePlace(), $filter ) === false)
			continue;
		//-- Counter
		echo "<tr>";
		...
		if ($tiny) {
			echo "<td class=\"list_value_wrap\">";
			if (showFactDetails( 'NCHI', $family->getXref() )) {
				if ($SEARCH_SPIDER) {
		...
		if ($husb->isDead() && !$wife->isDead()) {
			if ($wife->getSex() == "F")
				echo "H";
			if ($wife->getSex() == "M")
				echo "W"; // male partners
		}
		if (!$husb->isDead() && $wife->isDead()) {
			if ($husb->getSex() == "M")
				echo "W";
			if ($husb->getSex() == "F")
				echo "H"; // female partners
		}
		if (!$husb->isDead() && !$wife->isDead())
			echo "N";
		echo "</td>";
...
function print_sour_table( $datalist, $legend = null )
{
	...
	foreach ( $datalist as $key => $value ) {
		if (is_object( $value )) { // Array of objects
			...
			$source = isset( $value['gedcom'] ) ? new Source( $value['gedcom'] ) : Source::getInstance( $gid );
		}
...
function print_media_table( $datalist, $legend = "" )
{
	...
	echo "<td></td>";
	if ($SHOW_ID_NUMBERS)
		echo "<th class=\"list_label rela\">OBJE</th>";
	echo "<th class=\"list_label\">", $factarray["TITL"], "</th>";
	echo "<th class=\"list_label\">", $pgv_lang["individuals"], "</th>";
	echo "<th class=\"list_label\">", $pgv_lang["families"], "</th>";
	echo "<th class=\"list_label\">", $pgv_lang["sources"], "</th>";
...
function print_surn_table( $datalist, $target = "INDI", $listFormat = "" )
{
	...
	if ($listFormat == "style3") {
		...
		foreach ( $datalist as $key => $value ) {
			...
			$url = encode_url( $url );
			if (empty( $surn ) || trim( "@" . $surn, "_" ) == "@" || $surn == "@N.N.")
				$surn = $pgv_lang["NN"];
...
function format_surname_table( $surnames, $type )
{
	...
	foreach ( $surnames as $surn => $surns ) {
		...
		$url = $surn ? $type . '.php?surname=' . urlencode( $surn ) . '&amp;ged=' . urlencode( $GEDCOM ) : $type . '.php?alpha=,&amp;ged=' . urlencode( $GEDCOM );
		// Row counter
		++$row_num;
...
function format_surname_tagcloud( $surnames, $type, $totals )
{
	...
	foreach ( $surnames as $surn => $surns ) {
		...
		foreach ( $surns as $spfxsurn => $indis ) {
			...
			$total = $totals ? '(' . $count . ')' : '';
			$tag = ( $TEXT_DIRECTION == "ltr" ) ? "<font size=\"" . $fontsize . "\">" . PrintReady( $spfxsurn ) . "</font><span class=\"tag_cloud_sub\">&nbsp;" . $total . "</span>" : PrintReady( "<span class=\"tag_cloud_sub\">" . getRLM() . $total . getRLM() . "&nbsp;</span><font size=\"" . $fontsize . "\">" . $spfxsurn . "</font>" );
...
function format_surname_list( $surnames, $style, $totals )
{
	...
	foreach ( $surnames as $surn => $surns ) {
		...
		$url = $surn ? 'indilist.php?surname=' . urlencode( $surn ) . '&amp;ged=' . urlencode( $GEDCOM ) : 'indilist.php?alpha=,&amp;ged=' . urlencode( $GEDCOM );
		// If all the surnames are just case variants, then merge them into one
		// Comment out this block if you want SMITH listed separately from Smith
		$first_spfxsurn = null;
		foreach ( $surns as $spfxsurn => $indis ) {
...
function print_changes_table( $datalist, $showChange = true, $total = '', $show_pgvu = true )
{
	...
	//echo "<td></td>";
	if ($SHOW_ID_NUMBERS)
		echo "<th class=\"list_label rela\">", $pgv_lang["id"], "</th>";
	...
	if ($hidden)
		echo "<br /><span class=\"warning\">", $pgv_lang["hidden"], " : ", $hidden, "</span>";
	if ($n >= $NMAX)
		echo "<br /><span class=\"warning\">", $pgv_lang["recent_changes"], " &gt; ", $NMAX, "</span>";
	echo "</td>";
	echo "<td style=\"display:none\">GIVN</td>";
	echo "<td></td>";
	echo "<td></td>";
...
function print_events_table( $startjd, $endjd, $events = 'BIRT MARR DEAT', $only_living = false, $allow_download = false, $sort_by_event = false )
{
	...
	foreach ( get_events_list( $startjd, $endjd, $events ) as $value ) {
		...
		$value['sex'] = ( $record->getType() == "INDI" ) ? $record->getSexImage() : '';
		$filtered_events[] = $value;
	}

	// Now we've filtered the list, we can sort by event, if required
	if ($sort_by_event == "anniv") {
		uasort( $filtered_events, '\Bitweaver\Phpgedview\event_sort' );
	}
	elseif ($sort_by_event) {
		uasort( $filtered_events, '\Bitweaver\Phpgedview\event_sort_name' );
	}
	...
	foreach ( $filtered_events as $value ) {
		...
		if ($value['record']->getType() == "FAM") {
			...
			if ($husb)
				$exp[0] .= $husb->getPrimaryParentsNames( "parents_$table_id details1", "none" );
			$wife = $value['record']->getWife();
			if ($wife)
				$exp[1] .= $wife->getPrimaryParentsNames( "parents_$table_id details1", "none" );
...
//		}
	...
	if ($endjd == client_jd()) {
		...
		if ($private != 0) {
			...
			$summary = ( $output != 0 ) ? "more_today_privacy" : "none_today_privacy";
		}
		else if ($filter != 0) {
			// We lost some output due to filtering for living people
			if ($output == 0)
				$summary = "none_today_living";
		} else if ($output == 0)
			$summary = "none_today_all";
	} else {
		// We're dealing with the Upcoming Events block
		if ($private != 0) {
			// We lost some output due to Privacy restrictions
			$summary = ( $output != 0 ) ? "more_events_privacy" : "no_events_privacy";
		} else if ($filter != 0) {
			// We lost some output due to filtering for living people
			if ($output == 0)
				$summary = "no_events_living";
		} else if ($output == 0)
			$summary = "no_events_all";
		// If we're only looking at tomorrow, change the messages to refer
		// to tomorrow instead of "the next 1 days"
		if ($summary != "" && $endjd == $startjd)
			$summary .= "1";
	}
	if ($summary != "") {
		$return .= "<b>";
...
function print_events_list( $startjd, $endjd, $events = 'BIRT MARR DEAT', $only_living = false, $sort_by_event = false )
{
	...
	foreach ( $filtered_events as $value ) {
		...
		if (!empty( $value['plac'] ))
			$return .= " - <a href=\"" . encode_url( get_place_url( $value['plac'] ) ) . "\">" . $value['plac'] . "</a>";
		$return .= "</div>";
	}
...
function print_chart_by_age($data, $title) {
	...
	if ($count<1) return;
	$avg = round($avg/$count);
	$chart_url = PHPGEDVIEW_PKG_URL."chart.php?cht=bvs"; // chart type
