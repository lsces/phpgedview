...
function stripLRMRLM($inputText) {
	return str_replace([ PGV_UTF8_LRM, PGV_UTF8_RLM, PGV_UTF8_LRO, PGV_UTF8_RLO, PGV_UTF8_LRE, PGV_UTF8_RLE, PGV_UTF8_PDF, "&lrm;", "&rlm;", "&LRM;", "&RLM;" ], "", $inputText ?? '' );
}
	...
	while ($workingText != '') {
		...
		case '&':
			...
			if (substr($entity, 0, 2 == '&#')) {
				...
				$ordinal = (substr($entity, 2, 1) == 'x') || (substr($entity, 2, 1) == 'X')
					// the entity is a hexadecimal number
					? hexdec(substr($entity, 3, -1))
					// the entity is a decimal number
					: intval(substr($entity, 2, -1));
				if ($ordinal == 10) {
				...
				if (strpos($numbers, $charArray['letter']) === false) {
					...
					if (strpos($numbers, $charArray['letter']) === false) {
						...
						if (strpos($numberPrefix, $currentLetter) === false) {
							...
							$currentLetter .= PGV_UTF8_PDF;		// Include a trailing + or - in the run
						}
					}
				}
			} else {
...
function reverseText($text) {
	...
	while ($text!='') {
		...
		if (in_array($letter, $UTF8_numbers)) $numbers .= $letter;		// accumulate numbers in LTR mode
			...
			$numbers = '';
			 $reversedText = isset($UTF8_brackets[$letter]) ? $UTF8_brackets[$letter].$reversedText	: $letter.$reversedText;
		}
	}
