...
////////////////////////////////////////////////////////////////////////////////
$all_dirs=array(PHPGEDVIEW_PKG_INDEX_PATH =>'');
...
if (count($all_geds)==0) {
	...
	uasort($all_geds, "\Bitweaver\Phpgedview\stringsort");
}
...
$fam_facts_unique = ( isset( $FAM_FACTS_UNIQUE ) ) ? preg_split( '/[, ;:]+/', $FAM_FACTS_UNIQUE, -1, PREG_SPLIT_NO_EMPTY ) : array();

$indi_facts_unique = ( isset( $INDI_FACTS_UNIQUE ) ) ? preg_split( '/[, ;:]+/', $INDI_FACTS_UNIQUE, -1, PREG_SPLIT_NO_EMPTY ) : array();
...
function missing( $text )
{
	...
	return $pgv_lang['missing'] . ' &lrm;' . $text . ' &lrm;';
}

function multiple( $text )
{
	global $pgv_lang;
	return $pgv_lang['multiple'] . ' &lrm;' . $text . ' &lrm;';
}

function invalid( $text )
{
	global $pgv_lang;
	return $pgv_lang['invalid'] . ' &lrm;' . $text . ' &lrm;';
}

function too_many( $text )
{
	global $pgv_lang;
	return $pgv_lang['too_many'] . ' &lrm;' . $text . ' &lrm;';
}

function too_few( $text )
{
	global $pgv_lang;
	return $pgv_lang['too_few'] . ' &lrm;' . $text . ' &lrm;';
}

function no_link( $text )
{
	global $pgv_lang;
	return '&lrm;' . $text . '&lrm; ' . $pgv_lang['no_link'];
}
...
if ($num_lines > 0)
	$gedfile[0] = preg_replace( '/^' . PGV_UTF8_BOM . '/', '', $gedfile[0] );
$context = array( 'GEDCOM' );
$curr_xref = '';
foreach ( $gedfile as $num => $value ) {
	...
	$linked_rec = ( isset( $XREF_LINK[$tag] ) ) ? $XREF_LINK[$tag] : $tag;
