...
elseif ($action=="setup") {
	...
	if ($ENABLE_AUTOCOMPLETE) {
		require_once PGV_ROOT."js/autocomplete.js.html";
	}
	...
	if (!file_exists($report)) {
		...
		$xmlHandler = new \Bitweaver\Phpgedview\XMLHandler();
		...
		echo "<tr><td class=\"descriptionbox width30 wrap\">", $pgv_lang["selected_report"], "</td><td class=\"optionbox\">", $report_array["title"] ?? 'Not set', "</td></tr>\n";

		$doctitle = trim($report_array["title"] ?? 'Not set');
				...
				if (($input["name"] != "photos") || $MULTI_MEDIA) {
					...
					if (isset($input["lookup"])) {
						if ($input["lookup"]=="INDI") {
							$input["default"] = !empty($pid) ? $pid : check_rootid($input["default"]);
						}
						if ($input["lookup"]=="FAM") {
							if (!empty($famid)) {
								$input["default"] = $famid;
					...
					if (isset($input["lookup"])) {
						...
						if ($input["lookup"]=="INDI") {
							...
							$Link = isset($PGV_IMAGES["calendar"]["button"])
								? "<img src=\"".$PGV_IMAGE_DIR."/".$PGV_IMAGES["calendar"]["button"]."\" name=\"a_".$input["name"]."\" id=\"a_".$input["name"]."\" alt=\"".$text."\" title=\"".$text."\" border=\"0\" align=\"middle\" />"
								: $text;
							?>
...
global $xmlHandler, $PGVReportRoot;
	...
	$xml_parser = xml_parser_create();
	$xmlHandler = new ReportHandler($xml_parser, $PGVReportRoot );
	$xmlHandler->vars = $vars;
	$xmlHandler->report = $report;
	$xmlHandler->type = $type ?? [];
